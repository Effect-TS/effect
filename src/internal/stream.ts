...
>(
  ...
  <R, E, A, R2, E2, A2>(
    self: Stream<R, E, A>,
    that: Stream<R2, E2, A2>
  ): Stream<R | R2, E | E2, readonly [A, A2]> => pipe(self, crossWith(that, (a, a2) => [a, a2]))
)
    ...
    Effect.flatMap((queuesRef) =>
      Effect.gen(function*($) {
        ...
        const newQueue = yield* $(
          Ref.make<Effect<never, never, readonly [number, Queue<Exit<Option<E>, A>>]>>(
            pipe(
...
export const groupAdjacentBy = dual<
  ...
  <R, E, A, K>(
    self: Stream<R, E, A>,
    f: (a: A) => K
  ) => Stream<R, E, readonly [K, Chunk.NonEmptyChunk<A>]>
>(
  2,
  <R, E, A, K>(
    self: Stream<R, E, A>,
    ...
    const groupAdjacentByChunk = (
      state: Option<Output>,
      chunk: Chunk<A>
    ): readonly [Option<Output>, Chunk<Output>] => {
...
export const peel = dual<
  <R2, E2, A, Z>(
    ...
    self: Stream<R, E, A>
  ) => Effect<Scope | R2 | R, E2 | E, readonly [Z, Stream<never, E, A>]>,
  <R, E, R2, E2, A, Z>(
    self: Stream<R, E, A>,
    sink: Sink<R2, E2, A, A, Z>
  ) => Effect<Scope | R2 | R, E2 | E, readonly [Z, Stream<never, E, A>]>
>(2, <R, E, R2, E2, A, Z>(
  self: Stream<R, E, A>,
  sink: Sink<R2, E2, A, A, Z>
): Effect<R | R2 | Scope, E2 | E, readonly [Z, Stream<never, E, A>]> => {
...
export const partitionEither = dual<
  <A, R2, E2, A2, A3>(
    ...
    Scope | R2 | R,
    E2 | E,
    readonly [Stream<never, E2 | E, A2>, Stream<never, E2 | E, A3>]
  >,
  <R, E, A, R2, E2, A2, A3>(
    self: Stream<R, E, A>,
    predicate: (a: A) => Effect<R2, E2, Either<A2, A3>>,
...
>(
  ...
  <R, E, A, R2, E2, A2>(
    self: Stream<R, E, A>,
    that: Stream<R2, E2, A2>
  ): Stream<R | R2, E | E2, readonly [A, A2]> => pipe(self, zipWith(that, (a, a2) => [a, a2]))
)
...
export const zipAllSortedByKeyWith = dual<
  ...
  ) => Stream<R2 | R, E2 | E, readonly [K, A3]>
>(
  2,
  <R, E, R2, E2, A, A3, A2, K>(
    self: Stream<R, E, readonly [K, A]>,
...
>(
  ...
  <R, E, A, R2, E2, A2>(
    self: Stream<R, E, A>,
    that: Stream<R2, E2, A2>
  ): Stream<R | R2, E | E2, readonly [A, A2]> => pipe(self, zipLatestWith(that, (a, a2) => [a, a2]))
)
...
export const zipWithNext = <R, E, A>(
  self: Stream<R, E, A>
): Stream<R, E, readonly [A, Option<A>]> => {
  const process = (
    last: Option<A>
...
export const zipWithPrevious = <R, E, A>(
  self: Stream<R, E, A>
): Stream<R, E, readonly [Option<A>, A]> =>
  pipe(
    self,
    mapAccum<Option<A>, A, readonly [Option<A>, A]>(
      Option.none(),
...
const zipChunks = <A, B, C>(
  ...
  right: Chunk<B>,
  f: (a: A, b: B) => C
): readonly [Chunk<C>, Either<Chunk<A>, Chunk<B>>] => {
  if (left.length > right.length) {
    return [
