...
export const collectAllUntil = <In>(p: Predicate<In>): Sink<never, never, In, In, Chunk<In>> => {
  return pipe(
    fold<readonly [Chunk<In>, boolean], In>(
      [Chunk.empty(), true],
      (tuple) => tuple[1],
...
export const collectAllUntilEffect = <In, R, E>(p: (input: In) => Effect<R, E, boolean>) => {
  return pipe(
    foldEffect<readonly [Chunk<In>, boolean], R, E, In>(
      [Chunk.empty(), true],
      (tuple) => tuple[1],
...
export const collectLeftover = <R, E, In, L, Z>(
  self: Sink<R, E, In, L, Z>
): Sink<R, E, In, never, readonly [Z, Chunk<L>]> =>
...
const foldChunkSplitEffect = <S, R, E, In>(
  ...
  contFn: Predicate<S>,
  f: (s: S, input: In) => Effect<R, E, S>
): Effect<R, E, readonly [S, Option<Chunk<In>>]> => foldChunkSplitEffectInternal(s, chunk, 0, chunk.length, contFn, f)
...
const foldChunkSplitEffectInternal = <S, R, E, In>(
  ...
  contFn: Predicate<S>,
  f: (s: S, input: In) => Effect<R, E, S>
): Effect<R, E, readonly [S, Option<Chunk<In>>]> => {
  if (index === length) {
...
>(
  ...
  <R, E, R2, E2, In, In2 extends In, L, L2, Z, Z2>(
    ...
    that: Sink<R2, E2, In2, L2, Z2>,
    options?: { readonly concurrent?: boolean }
  ): Sink<R | R2, E | E2, In & In2, L | L2, readonly [Z, Z2]> => zipWith(self, that, (z, z2) => [z, z2], options)
)
