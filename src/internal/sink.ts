...
/** @internal */
export class SinkImpl<R, E, In, L, Z> implements Sink<R, E, In, L, Z> {
...
/** @internal */
export const isSink = (u: unknown): u is Sink<unknown, unknown, unknown, unknown, unknown> => hasProperty(u, SinkTypeId)
...
>(
  2,
  <R, E, In, L, Z, Z2>(self: Sink<R, E, In, L, Z>, z: Z2): Sink<R, E, In, L, Z2> => pipe(self, map(() => z))
)

/** @internal */
export const collectAll = <In>(): Sink<never, never, In, never, Chunk<In>> =>
...
const collectAllLoop = <In>(
  ...
  core.readWithCause({
    onInput: (chunk: Chunk<In>) => collectAllLoop(pipe(acc, Chunk.appendAll(chunk))),
    onFailure: core.failCause,
    onDone: () => core.succeed(acc)
  })

/** @internal */
export const collectAllN = <In>(n: number): Sink<never, never, In, In, Chunk<In>> =>
...
const collectAllNLoop = <In>(
  n: number,
  acc: Chunk<In>
): Channel<never, never, Chunk<In>, unknown, never, Chunk<In>, Chunk<In>> =>
  core.readWithCause({
    onInput: (chunk: Chunk<In>) => {
...
export const collectAllToMapN = <In, K>(
  ...
  return foldWeighted<HashMap<K, In>, In>({
...
/** @internal */
export const collectAllToSet = <In>(): Sink<never, never, In, never, HashSet<In>> =>
  foldLeftChunks<HashSet<In>, In>(
    HashSet.empty(),
    (acc, chunk) => pipe(chunk, Chunk.reduce(acc, (acc, input) => pipe(acc, HashSet.add(input))))
  )

/** @internal */
export const collectAllToSetN = <In>(n: number): Sink<never, never, In, In, HashSet<In>> =>
  foldWeighted<HashSet<In>, In>({
...
/** @internal */
export const collectAllUntil = <In>(p: Predicate<In>): Sink<never, never, In, In, Chunk<In>> => {
  return pipe(
    fold<[Chunk<In>, boolean], In>(
      [Chunk.empty(), true],
      (tuple) => tuple[1],
...
export const collectAllUntilEffect = <In, R, E>(p: (input: In) => Effect<R, E, boolean>) => {
  return pipe(
    foldEffect<[Chunk<In>, boolean], R, E, In>(
      [Chunk.empty(), true],
      (tuple) => tuple[1],
...
): Sink<R, E, In, In, Chunk<In>> => fromChannel(collectAllWhileEffectReader(predicate, Chunk.empty()))

/** @internal */
const collectAllWhileEffectReader = <In, R, E>(
  predicate: (input: In) => Effect<R, E, boolean>,
  done: Chunk<In>
): Channel<R, never, Chunk<In>, unknown, E, Chunk<In>, Chunk<In>> =>
  core.readWith({
    onInput: (input: Chunk<In>) =>
      pipe(
  ...
  <R, E, In, L extends In, Z, S>(
    ...
    const newChannel = pipe(
      ...
      core.flatMap(([leftoversRef, upstreamDoneRef]) => {
        const upstreamMarker: Channel<never, never, Chunk<In>, unknown, never, Chunk<In>, unknown> = core.readWith({
    ...
    channel.foldChannel({
      ...
      onSuccess: ([leftovers, doneValue]) =>
        p(doneValue)
          ? pipe(
            core.fromEffect(
              Ref.set(leftoversRef, Chunk.flatten(leftovers as Chunk<Chunk<In>>))
            ),
            core.flatMap(() =>
              pipe(
...
>(
  ...
  <R, E, L, Z, In0, In>(
    ...
    const loop: Channel<R, never, Chunk<In0>, unknown, never, Chunk<In>, unknown> = core.readWith({
...
>(
  ...
  <R, E, L, Z, In0, R2, E2, In>(
    ...
    const loop: Channel<R | R2, never, Chunk<In0>, unknown, E2, Chunk<In>, unknown> = core
      .readWith({
...
/** @internal */
export const die = (defect: unknown): Sink<never, never, unknown, never, never> => failCause(Cause.die(defect))

/** @internal */
export const dieMessage = (message: string): Sink<never, never, unknown, never, never> =>
  failCause(Cause.die(Cause.RuntimeException(message)))

/** @internal */
export const dieSync = (evaluate: LazyArg<unknown>): Sink<never, never, unknown, never, never> =>
...
>(
  ...
  ): Sink<R, E, In0, L, Z2> => map(mapInput(self, options.onInput), options.onDone)
)
...
/** @internal */
export const drain: Sink<never, never, unknown, never, void> = new SinkImpl(
  channel.drain(channel.identityChannel<never, unknown, unknown>())
)

/** @internal */
export const drop = <In>(n: number): Sink<never, never, In, In, unknown> => suspend(() => new SinkImpl(dropLoop(n)))

/** @internal */
const dropLoop = <In>(
  n: number
): Channel<never, never, Chunk<In>, unknown, never, Chunk<In>, unknown> =>
  core.readWith({
    onInput: (input: Chunk<In>) => {
      ...
      return pipe(
        core.write(dropped),
        channel.zipRight(channel.identityChannel<never, Chunk<In>, unknown>())
      )
    },
    onFailure: core.fail,
    onDone: () => core.unit
...
/** @internal */
export const dropUntil = <In>(predicate: Predicate<In>): Sink<never, never, In, In, unknown> =>
  new SinkImpl(
...
export const dropUntilEffect = <In, R, E>(
  predicate: (input: In) => Effect<R, E, boolean>
): Sink<R, E, In, In, unknown> => suspend(() => new SinkImpl(dropUntilEffectReader(predicate)))

/** @internal */
const dropUntilEffectReader = <In, R, E>(
  predicate: (input: In) => Effect<R, E, boolean>
): Channel<R, E, Chunk<In>, unknown, E, Chunk<In>, unknown> =>
  core.readWith({
    onInput: (input: Chunk<In>) =>
      pipe(
        input,
        ...
        Effect.map((leftover) => {
          ...
          return more ?
            ...
            pipe(
              ...
              channel.zipRight(channel.identityChannel<E, Chunk<In>, unknown>())
            )
        }),
        channel.unwrap
      ),
...
/** @internal */
export const dropWhile = <In>(predicate: Predicate<In>): Sink<never, never, In, In, unknown> =>
...
const dropWhileReader = <In>(
  ...
  core.readWith({
    onInput: (input: Chunk<In>) => {
      ...
      return pipe(core.write(out), channel.zipRight(channel.identityChannel<never, Chunk<In>, unknown>()))
    },
    onFailure: core.fail,
...
export const dropWhileEffect = <In, R, E>(
  predicate: (input: In) => Effect<R, E, boolean>
): Sink<R, E, In, In, unknown> => suspend(() => new SinkImpl(dropWhileEffectReader(predicate)))
...
>(
  ...
  <R, E, In, L, Z, R2, _>(
    self: Sink<R, E, In, L, Z>,
    finalizer: Effect<R2, never, _>
  ): Sink<R | R2, E, In, L, Z> => new SinkImpl(pipe(self, toChannel, channel.ensuring(finalizer)))
)
...
>(
  ...
  <R, In, L, E, Z, R2, _>(
    self: Sink<R, E, In, L, Z>,
    finalizer: (exit: Exit<E, Z>) => Effect<R2, never, _>
  ): Sink<R | R2, E, In, L, Z> => new SinkImpl(pipe(self, toChannel, core.ensuringWith(finalizer)))
)

/** @internal */
export const context = <R>(): Sink<R, never, unknown, never, Context<R>> => fromEffect(Effect.context<R>())

/** @internal */
export const contextWith = <R, Z>(
  f: (context: Context<R>) => Z
): Sink<R, never, unknown, never, Z> => pipe(context<R>(), map(f))

/** @internal */
export const contextWithEffect = <R0, R, E, Z>(
  f: (context: Context<R0>) => Effect<R, E, Z>
): Sink<R0 | R, E, unknown, never, Z> => pipe(context<R0>(), mapEffect(f))
...
/** @internal */
export const every = <In>(predicate: Predicate<In>): Sink<never, never, In, In, boolean> =>
  fold(true, identity, (acc, input) => acc && predicate(input))

/** @internal */
export const fail = <E>(e: E): Sink<never, E, unknown, never, never> => new SinkImpl(core.fail(e))

/** @internal */
export const failSync = <E>(evaluate: LazyArg<E>): Sink<never, E, unknown, never, never> =>
  new SinkImpl(core.failSync(evaluate))

/** @internal */
export const failCause = <E>(cause: Cause<E>): Sink<never, E, unknown, never, never> =>
...
export const filterInput: {
  ...
  return <R, E, L, Z>(self: Sink<R, E, In, L, Z>): Sink<R, E, In1, L, Z> => pipe(self, mapInputChunks(Chunk.filter(f)))
}
  ...
  <R, E, In, L extends In, Z, R2, E2>(
    ...
    const newChannel = pipe(
      ...
      core.flatMap(([leftoversRef, upstreamDoneRef]) => {
        ...
        const loop: Channel<R | R2, never, Chunk<In>, unknown, E | E2, Chunk<L>, Option<Z>> = channel.foldChannel(
...
export const fold = <S, In>(
  ...
  contFn: Predicate<S>,
  f: (z: S, input: In) => S
): Sink<never, never, In, In, S> => suspend(() => new SinkImpl(foldReader(s, contFn, f)))
...
export const foldSink = dual<
  ...
  ) => Sink<R1 | R2 | R, E1 | E2, In1 & In2, L1 | L2, Z1 | Z2>
>(
  2,
  <R, R1, R2, E, E1, E2, In, In1 extends In, In2 extends In, L, L1, L2, Z, Z1, Z2>(
    self: Sink<R, E, In, L, Z>,
    ...
    > = pipe(
      ...
      channel.foldChannel({
        ...
        onSuccess: ([leftovers, z]) =>
          core.suspend(() => {
            const leftoversRef = {
              ref: pipe(leftovers, Chunk.filter(Chunk.isNonEmpty)) as Chunk<Chunk<L1 | L2>>
            }
            const refReader = pipe(
              ...
              core.flatMap((chunk) => channel.writeChunk(chunk as Chunk<Chunk<In1 & In2>>))
            )
            const passthrough = channel.identityChannel<never, Chunk<In1 & In2>, unknown>()
...
): Sink<never, never, In, never, S> => suspend(() => new SinkImpl(foldChunksReader(s, contFn, f)))
...
const foldChunksReader = <S, In>(
  ...
  return core.readWith({
    onInput: (input: Chunk<In>) => foldChunksReader(f(s, input), contFn, f),
    onFailure: core.fail,
    onDone: () => core.succeedNow(s)
  })
}
...
): Sink<R, E, In, In, S> => suspend(() => new SinkImpl(foldChunksEffectReader(s, contFn, f)))
...
): Sink<R, E, In, In, S> => suspend(() => new SinkImpl(foldEffectReader(s, contFn, f)))
...
const foldChunkSplitEffect = <S, R, E, In>(
  ...
  contFn: Predicate<S>,
  f: (s: S, input: In) => Effect<R, E, S>
): Effect<R, E, [S, Option<Chunk<In>>]> => foldChunkSplitEffectInternal(s, chunk, 0, chunk.length, contFn, f)
...
/** @internal */
export const foldLeft = <S, In>(s: S, f: (s: S, input: In) => S): Sink<never, never, In, never, S> =>
...
export const foldLeftChunks = <S, In>(
  s: S,
  f: (s: S, chunk: Chunk<In>) => S
): Sink<never, never, In, never, S> => foldChunks(s, constTrue, f)

/** @internal */
export const foldLeftChunksEffect = <S, R, E, In>(
  s: S,
  f: (s: S, chunk: Chunk<In>) => Effect<R, E, S>
): Sink<R, E, In, never, S> => ignoreLeftover(foldChunksEffect(s, constTrue, f))

/** @internal */
export const foldLeftEffect = <S, R, E, In>(
  s: S,
  f: (s: S, input: In) => Effect<R, E, S>
): Sink<R, E, In, In, S> => foldEffect(s, constTrue, f)

/** @internal */
export const foldUntil = <S, In>(s: S, max: number, f: (z: S, input: In) => S): Sink<never, never, In, In, S> =>
  pipe(
    fold<[S, number], In>(
      [s, 0],
      (tuple) => tuple[1] < max,
...
const foldWeightedDecomposeEffectFold = <S, In, R, E, R2, E2, R3, E3>(
  ...
  cost: number,
  index: number
): Effect<R | R2 | R3, E | E2 | E3, [S, number, boolean, Chunk<In>]> => {
  if (index === input.length) {
...
>(
  ...
  <R, E, R1, E1, In, In1 extends In, L, L1, Z, Z1>(
    self: Sink<R, E, In, L, Z>,
    f: (z: Z) => Sink<R1, E1, In1, L1, Z1>
  ): Sink<R | R1, E | E1, In & In1, L | L1, Z1> => foldSink(self, { onFailure: fail, onSuccess: f })
)

/** @internal */
export const forEach = <In, R, E, _>(f: (input: In) => Effect<R, E, _>): Sink<R, E, In, never, void> => {
  const process: Channel<R, E, Chunk<In>, unknown, E, never, void> = core.readWithCause({
    onInput: (input: Chunk<In>) =>
...
export const forEachChunk = <In, R, E, _>(
  ...
  const process: Channel<R, E, Chunk<In>, unknown, E, never, void> = core.readWithCause({
    onInput: (input: Chunk<In>) => pipe(core.fromEffect(f(input)), core.flatMap(() => process)),
    onFailure: core.failCause,
    onDone: () => core.unit
  })
  return new SinkImpl(process)
...
export const forEachWhile = <In, R, E>(
  ...
  const process: Channel<R, E, Chunk<In>, unknown, E, Chunk<In>, void> = core.readWithCause({
    onInput: (input: Chunk<In>) => forEachWhileReader(f, input, 0, input.length, process),
    onFailure: core.failCause,
    onDone: () => core.unit
  })
  return new SinkImpl(process)
...
export const forEachChunkWhile = <In, R, E>(
  ...
  const reader: Channel<R, E, Chunk<In>, unknown, E, never, void> = core.readWith({
    onInput: (input: Chunk<In>) =>
      pipe(
...
): Sink<R, E, In, L, Z> => new SinkImpl(channel)

/** @internal */
export const fromEffect = <R, E, Z>(effect: Effect<R, E, Z>): Sink<R, E, unknown, never, Z> =>
...
): Sink<never, never, In, never, void> => fromQueue(pubsub, options)
...
export const fromPush = <R, E, In, L, Z>(
  ...
  >
): Sink<Exclude<R, Scope>, E, In, L, Z> => new SinkImpl(channel.unwrapScoped(pipe(push, Effect.map(fromPushPull))))

const fromPushPull = <R, E, In, L, Z>(
  push: (
    option: Option<Chunk<In>>
  ) => Effect<R, readonly [Either<E, Z>, Chunk<L>], void>
...
export const fromQueue = <In>(
  ...
  options?.shutdown ?
    ...
    forEachChunk((input: Chunk<In>) => pipe(Queue.offerAll(queue, input)))

/** @internal */
export const head = <In>(): Sink<never, never, In, In, Option<In>> =>
  fold(
...
/** @internal */
export const last = <In>(): Sink<never, never, In, In, Option<In>> =>
  foldLeftChunks(Option.none<In>(), (s, input) => Option.orElse(Chunk.last(input), () => s))

/** @internal */
export const leftover = <L>(chunk: Chunk<L>): Sink<never, never, unknown, L, void> =>
...
>(2, <R, E, In, L, Z, Z2>(self: Sink<R, E, In, L, Z>, f: (z: Z) => Z2): Sink<R, E, In, L, Z2> => {
...
>(
  ...
  <R, E, In, L, Z, R2, E2, Z2>(
    self: Sink<R, E, In, L, Z>,
    f: (z: Z) => Effect<R2, E2, Z2>
  ): Sink<R | R2, E | E2, In, L, Z2> => new SinkImpl(pipe(toChannel(self), channel.mapEffect(f)))
)
...
/** @internal */
export const never: Sink<never, never, unknown, never, never> = fromEffect(Effect.never)
...
>(
  ...
  <R, E, In, L, Z, R1, E1, In1, L1, Z1>(
    self: Sink<R, E, In, L, Z>,
    that: Sink<R1, E1, In1, L1, Z1>
  ): Sink<R | R1, E | E1, In & In1, L | L1, Z | Z1> => pipe(self, raceBoth(that), map(Either.merge))
)
...
export const raceBoth = dual<
  <R1, E1, In1, L1, Z1>(
    ...
    self: Sink<R, E, In, L, Z>
  ) => Sink<R1 | R, E1 | E, In & In1, L1 | L, Either<Z, Z1>>,
  <R, E, In, L, Z, R1, E1, In1, L1, Z1>(
    self: Sink<R, E, In, L, Z>,
    that: Sink<R1, E1, In1, L1, Z1>,
    options?: { readonly capacity?: number }
...
export const raceWith = dual<
  ...
  ) => Sink<R2 | R, E2 | E, In & In2, L2 | L, Z3 | Z4>
>(
  2,
  <R, In, L, R2, E2, In2, L2, Z2, E, Z, Z3, Z4>(
    self: Sink<R, E, In, L, Z>,
  ...
  ): Sink<R | R2, E | E2, In & In2, L | L2, Z3 | Z4> => {
    const scoped = Effect.gen(function*($) {
      const pubsub = yield* $(
        PubSub.bounded<Either<Exit<never, unknown>, Chunk<In & In2>>>(options?.capacity ?? 16)
      )
...
>(
  ...
  <R, In, L, Z, E, E2>(
    self: Sink<R, E, In, L, Z>,
    pf: (error: E) => Option<E2>
  ): Sink<R, E2, In, L, Z> => pipe(self, refineOrDieWith(pf, identity))
)
...
export const service = <T extends Context.Tag<any, any>>(
  tag: T
): Sink<Context.Tag.Identifier<T>, never, unknown, never, Context.Tag.Service<T>> => serviceWith(tag, identity)

/** @internal */
export const serviceWith = <T extends Context.Tag<any, any>, Z>(
  tag: T,
  f: (service: Context.Tag.Service<T>) => Z
): Sink<Context.Tag.Identifier<T>, never, unknown, never, Z> => fromEffect(Effect.map(tag, f))

/** @internal */
export const serviceWithEffect = <T extends Context.Tag<any, any>, R, E, Z>(
  tag: T,
  f: (service: Context.Tag.Service<T>) => Effect<R, E, Z>
): Sink<R | Context.Tag.Identifier<T>, E, unknown, never, Z> => fromEffect(Effect.flatMap(tag, f))
...
/** @internal */
export const some = <In>(predicate: Predicate<In>): Sink<never, never, In, In, boolean> =>
...
>(2, <R, E, L extends In, Z, In>(self: Sink<R, E, In, L, Z>, f: Predicate<In>): Sink<R, E, In, In, Z> => {
  const newChannel = pipe(
        ...
        core.flatMap(([leftovers, z]) =>
          pipe(
            ...
            core.flatMap((leftover) =>
              pipe(
                core.write<Chunk<In>>(pipe(leftover, Chunk.appendAll(Chunk.flatten(leftovers)))),
...
/** @internal */
const indexWhere = <A>(self: Chunk<A>, predicate: Predicate<A>, from = 0): number => {
...
/** @internal */
export const succeed = <Z>(z: Z): Sink<never, never, unknown, never, Z> => new SinkImpl(core.succeed(z))

/** @internal */
export const sum: Sink<never, never, number, never, number> = foldLeftChunks(
  0,
...
/** @internal */
export const sync = <Z>(evaluate: LazyArg<Z>): Sink<never, never, unknown, never, Z> =>
  new SinkImpl(core.sync(evaluate))

/** @internal */
export const take = <In>(n: number): Sink<never, never, In, In, Chunk<In>> =>
  pipe(
    foldChunks<Chunk<In>, In>(
      Chunk.empty(),
...
export const zip = dual<
  ...
  <R, E, R2, E2, In, In2 extends In, L, L2, Z, Z2>(
    ...
    that: Sink<R2, E2, In2, L2, Z2>,
    options?: { readonly concurrent?: boolean }
  ) => Sink<R2 | R, E2 | E, In & In2, L | L2, [Z, Z2]>
>(
  (args) => isSink(args[1]),
  <R, E, R2, E2, In, In2 extends In, L, L2, Z, Z2>(
    self: Sink<R, E, In, L, Z>,
    that: Sink<R2, E2, In2, L2, Z2>,
    options?: { readonly concurrent?: boolean }
  ): Sink<R | R2, E | E2, In & In2, L | L2, [Z, Z2]> => zipWith(self, that, (z, z2) => [z, z2], options)
)
...
>(
  ...
  <R, E, R2, E2, In, In2 extends In, L, L2, Z, Z2>(
    ...
    that: Sink<R2, E2, In2, L2, Z2>,
    options?: { readonly concurrent?: boolean }
  ): Sink<R | R2, E | E2, In & In2, L | L2, Z> => zipWith(self, that, (z, _) => z, options)
)
...
>(
  ...
  <R, E, R2, E2, In, In2 extends In, L, L2, Z, Z2>(
    ...
    that: Sink<R2, E2, In2, L2, Z2>,
    options?: { readonly concurrent?: boolean }
  ): Sink<R | R2, E | E2, In & In2, L | L2, Z2> => zipWith(self, that, (_, z2) => z2, options)
)
...
): Sink<Env, OutErr, InElem, OutElem, OutDone> => new SinkImpl(self)

// Constants

/** @internal */
export const count: Sink<never, never, unknown, never, number> = foldLeftChunks(
  0,
  (acc, chunk) => acc + chunk.length
)

/** @internal */
export const mkString: Sink<never, never, unknown, never, string> = suspend(() => {
...
/** @internal */
export const timed: Sink<never, never, unknown, never, Duration> = pipe(
  withDuration(drain),
  map((tuple) => tuple[1])
)
