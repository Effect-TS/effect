...
import type { KeyedPool } from "../exports/KeyedPool.js"
...
import type { Pool } from "../exports/Pool.js"
...
class KeyedPoolImpl<K, E, A> implements KeyedPool<K, E, A> {
  readonly [KeyedPoolTypeId] = keyedPoolVariance
  constructor(
    readonly getOrCreatePool: (key: K) => Effect<never, never, Pool<E, A>>,
    readonly activePools: () => Effect<never, never, Array<Pool<E, A>>>
  ) {}
...
class Pending<E, A> implements Equal {
  readonly _tag = "Pending"
  readonly [KeyedPoolMapValueSymbol]: KeyedPoolMapValueSymbol = KeyedPoolMapValueSymbol
  constructor(readonly deferred: Deferred<never, Pool<E, A>>) {}
  [Hash.symbol](): number {
    return pipe(
...
const makeImpl = <K, R, E, A>(
  ...
  min: (key: K) => number,
  max: (key: K) => number,
  timeToLive: (key: K) => Option<Duration>
): Effect<R | Scope, never, KeyedPool<K, E, A>> =>
  pipe(
    fiberRuntime.all([
      core.context<R>(),
      core.fiberId,
    ...
    core.map(([context, fiberId, map, scope]) => {
      const getOrCreatePool = (key: K): Effect<never, never, Pool<E, A>> =>
        core.suspend(() => {
          let value: MapValue<E, A> | undefined = Option.getOrUndefined(HashMap.get(MutableRef.get(map), key))
          if (value === undefined) {
            return core.uninterruptibleMask((restore) => {
              const deferred = core.deferredUnsafeMake<never, Pool<E, A>>(fiberId)
      ...
      const activePools = (): Effect<never, never, Array<Pool<E, A>>> =>
        core.suspend(() =>
