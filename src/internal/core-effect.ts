...
import type * as runtimeFlagsPatch from "../exports/RuntimeFlagsPatch.js"
import { Tracer } from "../exports/Tracer.js"
...
export const diffFiberRefs = <R, E, A>(
  self: Effect<R, E, A>
): Effect<R, E, [FiberRefsPatch, A]> => summarized(self, fiberRefs, fiberRefsPatch.diff)

/* @internal */
export const diffFiberRefsAndRuntimeFlags = <R, E, A>(
  self: Effect<R, E, A>
): Effect<R, E, [[FiberRefsPatch, runtimeFlagsPatch.RuntimeFlagsPatch], A]> =>
  summarized(
    self,
...
export const summarized = dual<
  <R2, E2, B, C>(
    ...
    f: (start: B, end: B) => C
  ) => <R, E, A>(self: Effect<R, E, A>) => Effect<R | R2, E | E2, [C, A]>,
  <R, E, A, R2, E2, B, C>(
    self: Effect<R, E, A>,
    summary: Effect<R2, E2, B>,
    f: (start: B, end: B) => C
...
export const timed = <R, E, A>(
  self: Effect<R, E, A>
): Effect<R, E, [Duration, A]> => timedWith(self, Clock.currentTimeNanos)

/* @internal */
export const timedWith = dual<
  <R1, E1>(
    nanoseconds: Effect<R1, E1, bigint>
  ) => <R, E, A>(self: Effect<R, E, A>) => Effect<R | R1, E | E1, [Duration, A]>,
  <R, E, A, R1, E1>(
    self: Effect<R, E, A>,
    nanoseconds: Effect<R1, E1, bigint>
...
export const whenFiberRef = dual<
  ...
  ) => <R, E, A>(self: Effect<R, E, A>) => Effect<R, E, [S, Option<A>]>,
  <R, E, A, S>(
    self: Effect<R, E, A>,
    fiberRef: FiberRef<S>,
    predicate: Predicate<S>
