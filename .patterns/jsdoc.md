...
- Use inline or standalone `// >` comments to assert the complete console output of a runnable example. Move inline markers to the next line when the source line would exceed 120 characters. Each marker represents one expected output line; examples without markers remain execution-only.
