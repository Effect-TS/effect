...
import { execute } from "../src/internal/kysely.js"
...
describe("MssqlKysely", () => {
  it.effect("queries", () =>
    Effect.gen(function*(_) {
      ...
      yield* execute(db.insertInto("users").values({ name: "Alice" }))
      const inserted = yield* execute(db.selectFrom("users").selectAll())
      yield* execute(db.updateTable("users").set({ name: "Bob", nickname: "The Bobinator" }))
      const updated = yield* execute(db.selectFrom("users").selectAll())
      yield* execute(db.deleteFrom("users"))
      const deleted = yield* execute(db.selectFrom("users").selectAll())
