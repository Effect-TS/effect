...
export class Sharding extends Context.Tag("@effect/cluster/Sharding")<Sharding, {
  ...
  /**
   * Returns `true` if sharding has acquired the given shard.
   */
  readonly hasShard: (shardId: ShardId) => boolean
...
export const make = Effect.gen(function*() {
  ...
  const hasShard = (shardId: ShardId) => acquiredShards.has(shardId)
