...
export const make = Effect.fnUntraced(function*(options?: {
  ...
  const tenSecondsLeft = sql.onDialectOrElse({
    pg: () => sql`${sqlNow} - INTERVAL '110 seconds'`,
    mysql: () => sql`DATE_SUB(${sqlNow}, INTERVAL 110 SECOND)`,
    mssql: () => sql`DATEADD(SECOND, -110, ${sqlNow})`,
    orElse: () => sql`datetime(${sqlNow}, '-110 seconds')`
  })
  ...
  return yield* ShardStorage.makeEncoded({
    ...
    release: (address, shardId) =>
      sql`UPDATE ${locksTableSql} SET acquired_at = ${tenSecondsLeft} WHERE address = ${address} AND shard_id = ${shardId}`
        .pipe(PersistenceError.refail),

    releaseAll: (address) =>
      sql`UPDATE ${locksTableSql} SET acquired_at = ${tenSecondsLeft} WHERE address = ${address}`.pipe(
        PersistenceError.refail
      )
  })
})
