...
class Ping extends Schema.Class<Ping>("Ping")({ id: Schema.String }) {
  [PrimaryKey.symbol]() {
    return this.id
  }
}
const TestEntity = Entity.make("ClusterIntegrationSmoke", [
  Rpc.make("Ping", { payload: Ping, success: Schema.String })
]).annotateRpcs(ClusterSchema.Persisted, true)
const TestEntityLayer = TestEntity.toLayer({ Ping: ({ payload }) => Effect.succeed(`pong:${payload.id}`) })

describe("cluster integration smoke", () => {
  for (const backend of ["pg", "mysql"] satisfies ReadonlyArray<Backend>) {
    it.scopedLive(`${backend}: persists messages and rebalances after an abrupt runner death`, () =>
      Effect.gen(function*() {
        ...
        assert.strictEqual(yield* client("entity-1").Ping(new Ping({ id: "request-1" })), "pong:request-1")
        ...
        yield* cluster.waitForStableAssignments()
        assert.strictEqual(yield* client("entity-1").Ping(new Ping({ id: "request-2" })), "pong:request-2")
