import { ClusterWorkflowEngine, Entity, EntityId, Sharding } from "@effect/cluster"
...
import { Cause, Clock, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import * as PersistedQueue from "../../../experimental/src/PersistedQueue.js"
import * as ClusterAbandon from "../../src/internal/clusterAbandon.js"
import { type Backend, type ClusterRunner, make } from "./harness.js"

const EndToEndWorkflow = Workflow.make({
  name: "ClusterIntegrationEndToEnd",
  payload: { id: Schema.String, value: Schema.Number },
  success: Schema.Number,
  idempotencyKey: ({ id }) => id
})
let endToEndGate = Effect.unsafeMakeLatch(true)
let endToEndEntered = Effect.unsafeMakeLatch()
...
const EndToEndWorkflowLayer = EndToEndWorkflow.toLayer(({ id, value }) =>
  Activity.make({
    ...
    execute: Effect.gen(function*() {
      ...
      yield* endToEndEntered.open
      yield* endToEndGate.await
      return value + 1
    })
  })
)
const ReplayGate = DurableDeferred.make("ClusterIntegrationReplayGate", { success: Schema.String })
const ReplayWorkflow = Workflow.make({
  name: "ClusterIntegrationReplay",
...
const ReplayWorkflowLayer = ReplayWorkflow.toLayer(Effect.fnUntraced(function*({ id }) {
  yield* Activity.make({
    ...
    execute: Effect.sync(() => {
      replayRuns.set(id, (replayRuns.get(id) ?? 0) + 1)
    })
  })
  return yield* DurableDeferred.await(ReplayGate)
}))
const RestartGate = DurableDeferred.make("ClusterIntegrationRestartGate", { success: Schema.String })
const RestartWorkflow = Workflow.make({
  name: "ClusterIntegrationRestart",
  payload: { id: Schema.String },
  success: Schema.String,
  idempotencyKey: ({ id }) => id
})
const RestartWorkflowLayer = RestartWorkflow.toLayer(() => DurableDeferred.await(RestartGate))
const RaceReplayGate = DurableDeferred.make("ClusterIntegrationRaceReplayGate", { success: Schema.String })
const RaceReplayTail = DurableDeferred.make("ClusterIntegrationRaceReplayTail", { success: Schema.String })
const RaceReplayWorkflow = Workflow.make({
...
class CompensationError
  extends Schema.TaggedError<CompensationError>()("ClusterIntegrationCompensationError", { id: Schema.String })
{}
const CompensationWorkflow = Workflow.make({
  name: "ClusterIntegrationCompensation",
  payload: { id: Schema.String },
...
const SuspendFailureWorkflow = Workflow.make({
...
const SuspendFailureWorkflowLayer = SuspendFailureWorkflow.toLayer(Effect.fnUntraced(function*({ id }) {
  ...
  return yield* Activity.make({
    name: "ClusterIntegrationSuspendFailure",
    execute: Effect.sync(() => suspendFailures.add(id)).pipe(Effect.andThen(Effect.die("suspend after owner loss")))
  })
}))
const RaceBoundaryGateA = DurableDeferred.make("ClusterIntegrationRaceBoundaryGateA", { success: Schema.String })
const RaceBoundaryGateB = DurableDeferred.make("ClusterIntegrationRaceBoundaryGateB", { success: Schema.String })
const RaceBoundaryWorkflow = Workflow.make({
  name: "ClusterIntegrationRaceBoundary",
  payload: { id: Schema.String },
  success: Schema.String,
  idempotencyKey: ({ id }) => id
})
let raceBoundaryCommitStarted = Effect.unsafeMakeLatch()
let raceBoundaryCommit = Effect.unsafeMakeLatch()
...
const RaceBoundaryWorkflowLayer = RaceBoundaryWorkflow.toLayer(Effect.fnUntraced(function*({ id }) {
  ...
  return yield* DurableDeferred.raceAll({
    ...
    success: Schema.String,
    error: Schema.Never,
    effects: [DurableDeferred.await(RaceBoundaryGateA), DurableDeferred.await(RaceBoundaryGateB)]
  }).pipe(Effect.ensuring(raceBoundaryCommitStarted.open.pipe(Effect.andThen(raceBoundaryCommit.await))))
}))
class RetryError extends Schema.TaggedError<RetryError>()("ClusterIntegrationRetryError", { attempt: Schema.Number }) {}
const RetryWorkflow = Workflow.make({
  name: "ClusterIntegrationRetry",
  payload: { id: Schema.String, succeed: Schema.Boolean },
  success: Schema.Number,
  error: RetryError,
  idempotencyKey: ({ id }) => id
})
...
const ClockWorkflowLayer = ClockWorkflow.toLayer(() =>
  Effect.gen(function*() {
    yield* DurableClock.sleep({ name: "RestartSleep", duration: "1 second", inMemoryThreshold: Duration.zero })
    return yield* Clock.currentTimeMillis
  })
)
const InterruptGate = DurableDeferred.make("ClusterIntegrationInterruptGate")
const InterruptWorkflow = Workflow.make({
  name: "ClusterIntegrationInterrupt",
  payload: { id: Schema.String },
  idempotencyKey: ({ id }) => id
})
...
const DeferredControl = Entity.make("ClusterIntegrationDeferredControl", [
  Rpc.make("CompleteDeferred", {
    payload: { token: DurableDeferred.Token, value: Schema.String },
    success: Schema.String
  }),
  Rpc.make("CompleteRaceBoundaryDeferred", {
    payload: { token: DurableDeferred.Token, value: Schema.String },
    success: Schema.String
  })
])
const DeferredControlLayer = DeferredControl.toLayer(Effect.gen(function*() {
  ...
  return {
    CompleteDeferred: ({ payload }) =>
      DurableDeferred.succeed(RestartGate, payload).pipe(Effect.as(`${runner.host}:${runner.port}`)),
    CompleteRaceBoundaryDeferred: ({ payload }) =>
      DurableDeferred.succeed(RaceBoundaryGateB, payload).pipe(Effect.as(`${runner.host}:${runner.port}`))
  }
}))
...
const QueueWorkflow = Workflow.make({
  name: "ClusterIntegrationQueue",
  payload: { id: Schema.String },
  success: Schema.String,
  idempotencyKey: ({ id }) => id
...
let queueWorkerGate = Effect.unsafeMakeLatch()
const QueueWorkflowLayer = QueueWorkflow.toLayer(({ id }) => DurableQueue.process(Queue, { id }))
const QueueWorkerLayer = Layer.scopedDiscard(Effect.forkScoped(
...
const ShutdownActivityWorkflow = Workflow.make({
  name: "ClusterIntegrationShutdownActivity",
  payload: { id: Schema.String },
  success: Schema.String,
  idempotencyKey: ({ id }) => id
})
...
const activityHandoffState = {
  ready: Effect.unsafeMakeLatch(),
  start: Effect.unsafeMakeLatch(),
  faultArmed: false,
  persisted: Effect.unsafeMakeLatch(),
  wasPersisted: false,
...
const resetActivityHandoffState = (id: string, faultReleaseOpen = true) => {
  activityHandoffState.ready = Effect.unsafeMakeLatch()
  activityHandoffState.start = Effect.unsafeMakeLatch()
  activityHandoffState.faultArmed = false
  activityHandoffState.persisted = Effect.unsafeMakeLatch()
...
const shutdownLayer = <Name extends string>(
  ...
  workflow.toLayer(({ id }) =>
    Effect.gen(function*() {
      yield* Effect.succeed(id).pipe(
        workflow.withCompensation(() => Effect.sync(() => activityHandoffState.compensations.add(id)))
      )
...
const ShutdownActivityWorkflowLayer = shutdownLayer(ShutdownActivityWorkflow)
          ...
          new Proxy(makeClient(entityId), {
            get(target, property, receiver) {
              ...
              return (payload: { readonly name: string }, options?: object) => {
                if (!activityHandoffState.faultArmed) return activity(payload, options)
                activityHandoffState.faultArmed = false
                return activity(payload, { ...options, discard: true }).pipe(
                  Effect.tap(() =>
                    Effect.sync(() => {
                      activityHandoffState.wasPersisted = true
                      activityHandoffState.persisted.unsafeOpen()
                    })
                  ),
                  Effect.andThen(activityHandoffState.faultRelease.await),
                  Effect.andThen(ClusterAbandon.interrupt),
...
const ShutdownActivityEntities = Layer.merge(
  ShutdownActivityWorkflowLayer,
  shutdownLayer(ShutdownSuspendActivityWorkflow)
).pipe(
  Layer.provide(Layer.scoped(WorkflowEngine.WorkflowEngine, ClusterWorkflowEngine.make)),
  Layer.provide(ActivityHandoffShardingLayer),
  Layer.orDie
)
type Cluster = Effect.Effect.Success<ReturnType<typeof make>>
const withWorkflow = <A, E, R>(cluster: Cluster, effect: Effect.Effect<A, E, R | WorkflowEngine.WorkflowEngine>) =>
  Effect.provideService(effect, WorkflowEngine.WorkflowEngine, cluster.workflowEngine)
const waitForSuspended = <
  Name extends string,
  Payload extends Workflow.AnyStructSchema,
  Success extends Schema.Schema.Any,
  Error extends Schema.Schema.All
>(cluster: Cluster, workflow: Workflow.Workflow<Name, Payload, Success, Error>, executionId: string) =>
  cluster.waitUntil(
    `${workflow.name}/${executionId} did not suspend`,
    Effect.map(workflow.poll(executionId), (result) => result?._tag === "Suspended")
  )
const waitForComplete = Effect.fnUntraced(
  function*<
    Name extends string,
    Payload extends Workflow.AnyStructSchema,
    Success extends Schema.Schema.Any,
    Error extends Schema.Schema.All
  >(cluster: Cluster, workflow: Workflow.Workflow<Name, Payload, Success, Error>, executionId: string) {
    let complete: Workflow.Complete<Schema.Schema.Type<Success>, Schema.Schema.Type<Error>> | undefined
    yield* cluster.waitUntil(
      `${workflow.name}/${executionId} did not complete`,
      Effect.map(workflow.poll(executionId), (result) => {
        if (result?._tag !== "Complete") return false
        complete = result
        return true
      })
    )
...
const restart = Effect.fnUntraced(function*(cluster: Cluster) {
  yield* Effect.forEach(cluster.runners.filter((r) => r.state() === "running"), cluster.kill, {
...
const workflowOwner = (cluster: Cluster, executionId: string) =>
  cluster.ownersOfShard(cluster.clientSharding.getShardId(EntityId.make(executionId), "default"))[0]
const findControlOnAnotherRunner = Effect.fnUntraced(function*(cluster: Cluster, runner: ClusterRunner) {
  for (let index = 0; index < 2_000; index++) {
    const id = `control-${index}`
    const owner = yield* cluster.ownerOfEntity(DeferredControl, id)
    if (owner !== undefined && owner !== runner) return [id, owner] as const
  }
...
describe("cluster workflow integration", () => {
  for (const backend of ["pg", "mysql"] satisfies ReadonlyArray<Backend>) {
    it.scopedLive(`${backend}: persists queued work across restart and consumes it once`, () =>
      Effect.gen(function*() {
        const id = `${backend}-queue`
        queueWorkerGate = Effect.unsafeMakeLatch()
    ...
    it.scopedLive(`${backend}: replays a persisted activity handoff without leaking workflow resources`, () =>
      Effect.gen(function*() {
        const cluster = yield* make({ backend, entities: ShutdownActivityEntities })
        yield* cluster.start(3)
        yield* cluster.waitForStableAssignments()
        for (
          const [suffix, workflow] of [["default", ShutdownActivityWorkflow], [
            "suspend-on-failure",
            ShutdownSuspendActivityWorkflow
          ]] as const
        ) {
          const id = `${backend}-shutdown-activity-${suffix}`
          resetActivityHandoffState(id)
          const executionId = yield* withWorkflow(cluster, workflow.execute({ id }, { discard: true }))
          yield* cluster.waitUntil(
          ...
          assert.isTrue(activityHandoffState.wasPersisted)
    ...
    it.scopedLive(`${backend}: preserves a safe interrupt during a persisted activity handoff`, () =>
      Effect.gen(function*() {
        const id = `${backend}-shutdown-activity-interrupt`
        resetActivityHandoffState(id, false)
        const cluster = yield* make({ backend, entities: ShutdownActivityEntities })
        yield* cluster.start(3)
        yield* Effect.addFinalizer(() => activityHandoffState.faultRelease.open)
        yield* cluster.waitForStableAssignments()
        const executionId = yield* withWorkflow(cluster, ShutdownActivityWorkflow.execute({ id }, { discard: true }))
        yield* cluster.waitUntil(
          "The shutdown activity workflow did not start",
          Effect.as(activityHandoffState.ready.await, true)
        )
    ...
    it.scopedLive(`${backend}: recovers a persisted activity across an actual owner handoff`, () =>
      Effect.gen(function*() {
        ...
        yield* cluster.waitUntil(
          ...
          Effect.sync(() => {
            const next = workflowOwner(cluster, executionId)
    ...
    it.scopedLive(`${backend}: executes end to end and deduplicates concurrent callers`, () =>
      Effect.gen(function*() {
    ...
    it.scopedLive(`${backend}: replays completed activities after the owner dies`, () =>
      Effect.gen(function*() {
    ...
    it.scopedLive(`${backend}: resumes deferred workflows across a whole-cluster restart from another runner`, () =>
      Effect.gen(function*() {
    ...
    it.scopedLive(`${backend}: wakes a DurableDeferred raceAll and replays the winner after owner death`, () =>
      Effect.gen(function*() {
        ...
        yield* withWorkflow(cluster, DurableDeferred.succeed(RaceReplayGate, { token: raceToken, value: "winner" }))
        ...
        yield* withWorkflow(
          cluster,
          DurableDeferred.succeed(RaceReplayTail, { token: tailToken, value: "after-owner-death" })
        )
    ...
    it.scopedLive(`${backend}: runs compensation and SuspendOnFailure after the owner is lost`, () =>
      Effect.gen(function*() {
        const compensationId = `${backend}-compensation-owner-loss`, suspendId = `${backend}-suspend-owner-loss`
        const cluster = yield* make({ backend, entities })
        yield* cluster.start(3)
        yield* cluster.waitForStableAssignments()
        ...
        const compensationOwner = workflowOwner(cluster, compensationExecutionId),
          suspendOwner = workflowOwner(cluster, suspendExecutionId)
        assert.isDefined(compensationOwner)
        assert.isDefined(suspendOwner)
        yield* Effect.forEach(new Set([compensationOwner!, suspendOwner!]), cluster.kill, { discard: true })
        yield* cluster.waitForStableAssignments()
        ...
        yield* withWorkflow(
          cluster,
          DurableDeferred.succeed(CompensationGate, { token: compensationToken, value: undefined })
        )
        const result = yield* waitForComplete(cluster, CompensationWorkflow, compensationExecutionId)
        ...
        const token = DurableDeferred.tokenFromExecutionId(SuspendFailureGate, {
          workflow: SuspendFailureWorkflow,
          executionId: suspendExecutionId
        })
        yield* withWorkflow(cluster, DurableDeferred.succeed(SuspendFailureGate, { token, value: undefined }))
        yield* cluster.waitUntil(
    ...
    it.scopedLive(`${backend}: accepts a late durable-race completion across the suspend commit`, () =>
      Effect.gen(function*() {
        const id = `${backend}-race-suspend-commit`
        raceBoundaryCommitStarted = Effect.unsafeMakeLatch()
        raceBoundaryCommit = Effect.unsafeMakeLatch()
        const cluster = yield* make({ backend, entities })
        yield* cluster.start(3)
        yield* Effect.addFinalizer(() => raceBoundaryCommit.open)
        yield* cluster.waitForStableAssignments()
        const executionId = yield* withWorkflow(cluster, RaceBoundaryWorkflow.execute({ id }, { discard: true }))
        yield* cluster.waitUntil(
        ...
        const completedBy = yield* control(controlId).CompleteRaceBoundaryDeferred({ token, value: "late-winner" })
        assert.strictEqual(completedBy, `${controlOwner.address.host}:${controlOwner.address.port}`)
        const killFiber = yield* cluster.kill(owner!).pipe(Effect.forkScoped)
        yield* raceBoundaryCommit.open
    ...
    it.scopedLive(`${backend}: applies activity retry policy and preserves the exhausted error`, () =>
      Effect.gen(function*() {
        const cluster = yield* make({ backend, entities })
        yield* cluster.start(3)
        yield* cluster.waitForStableAssignments()
        const successId = `${backend}-retry-success`, failureId = `${backend}-retry-failure`
        assert.strictEqual(yield* withWorkflow(cluster, RetryWorkflow.execute({ id: successId, succeed: true })), 3)
        const error = yield* withWorkflow(cluster, RetryWorkflow.execute({ id: failureId, succeed: false })).pipe(
          Effect.flip
        )
    ...
    it.scopedLive(`${backend}: wakes a durable clock after a whole-cluster restart`, () =>
      Effect.gen(function*() {
        const cluster = yield* make({ backend, entities })
        yield* cluster.start(3)
        yield* cluster.waitForStableAssignments()
    ...
    it.scopedLive(`${backend}: persists interruption across a whole-cluster restart`, () =>
      Effect.gen(function*() {
        const cluster = yield* make({ backend, entities })
        yield* cluster.start(3)
        yield* cluster.waitForStableAssignments()
        ...
        assert(Exit.isFailure(result.exit))
        assert.isTrue(Exit.isInterrupted(result.exit))
        assert.isTrue(Cause.isInterrupted(result.exit.cause))
      }))
  }
})
