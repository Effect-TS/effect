...
describe("cluster lock-storage integration", () => {
  for (const backend of ["pg", "mysql"] satisfies ReadonlyArray<Backend>) {
    it.scopedLive(`${backend}: rebuilds a blackholed reserved lock connection and rejoins without split brain`, () =>
      Effect.gen(function*() {
    ...
    it.scopedLive(`${backend}: does not reacquire a shard before forced release completes`, () =>
      Effect.gen(function*() {
        const cluster = yield* make({ backend, entities: Layer.empty, config: { shardsPerGroup: 4 } })
        const [faulted] = yield* cluster.start(1)
        yield* cluster.waitForStableAssignments()
        ...
        yield* cluster.waitUntil(
          ...
          Effect.sync(() => {
            const assignments = cluster.assignmentMap(), owners = Object.values(assignments)
            assert(owners.every((owners) => owners.length <= 1), "A shard had multiple owners")
            const complete = owners.every((owners) => owners.length === 1) && owners.some((owners) =>
              owners.includes(faultedAddress)
            ) && owners.some((owners) => owners.includes(peerAddress))
    ...
    it.scopedLive(`${backend}: keeps peers progressing when a lock query is permanently stuck`, () =>
      Effect.gen(function*() {
        const cluster = yield* make({ backend, entities: Layer.empty, config: { shardsPerGroup: 4 } })
        const [faulted] = yield* cluster.start(1)
        yield* cluster.waitForStableAssignments()
        ...
        yield* cluster.waitUntil(
          ...
          Effect.sync(() => {
            ...
            assert(owners.every((owners) => !owners.includes(faultedAddress)))
            return owners.some((owners) => owners.includes(peerAddress))
          })
        )
        yield* cluster.faultLock(faulted, "clear")
        yield* cluster.waitForStableAssignments()
      }))
    it.scopedLive(`${backend}: converges after repeated lock-connection rebuilds and a hung release`, () =>
      Effect.gen(function*() {
        const cluster = yield* make({ backend, entities: Layer.empty, config: { shardsPerGroup: 4 } })
        const [faulted] = yield* cluster.start(1)
        yield* cluster.waitForStableAssignments()
        ...
        yield* cluster.waitUntil(
          ...
          Effect.gen(function*() {
            yield* cluster.faultLock(faulted, "fail")
            assert(
              Object.values(cluster.assignmentMap()).every((owners) =>
                owners.length === 1 && owners.includes(faultedAddress)
              ),
