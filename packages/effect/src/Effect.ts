...
export interface EffectGenerator<T extends Effect<any, any, any>> {
  next(...args: ReadonlyArray<any>): IteratorResult<YieldKey<T>, Effect.Success<T>>
}
 ...
 */
export const gen: {
  <Eff extends YieldKey<Effect<any, any, any>>, AEff>(
    f: (resume: Adapter) => Generator<Eff, AEff, never>
  ): Effect<
    AEff,
    [Eff] extends [never] ? never : [Eff] extends [YieldKey<Effect<infer _A, infer E, infer _R>>] ? E : never,
    [Eff] extends [never] ? never : [Eff] extends [YieldKey<Effect<infer _A, infer _E, infer R>>] ? R : never
  >
  <Self, Eff extends YieldKey<Effect<any, any, any>>, AEff>(
    self: Self,
