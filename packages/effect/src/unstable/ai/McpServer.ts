    ...
    const notifications = yield* RpcClient.makeNoSerialization(ServerNotificationRpcs, {
      ...
      onFromClient: (options) =>
        Effect.suspend((): Effect.Effect<void> => {
          ...
          // RpcClient's callback erases the correlation between an RPC tag and
          // its decoded payload; restore the distributed notification union at
          // this single framework boundary.
          const notification = toInternalServerNotification(message as ServerNotificationRequest)
