...
export interface GenerateTextOptions<Tools extends Record<string, Tool.Any>> {
   ...
   * option. `Toolkit` routes a failure of its parameters through that tool's
   * own `failureMode`, and with `disableToolCallResolution: true` such a call
   * fails the operation as it does today.
   */
...
// a call to a tool which is not in the toolkit: such a call can never reach a
// handler, so no `failureMode` speaks for it. A call which names a tool in the
// toolkit is left to `Toolkit`, which routes a parameter failure through the
// tool's own `failureMode`, so parameters are never validated twice. Any other
// decode failure fails the operation exactly as it does by default. Every part
// decodes to exactly one part, so a non-empty chunk stays non-empty.
...
>(
  ...
  return Effect.forEach(parts, (part) =>
    Effect.flatMap(Effect.result(decodePart(part)), (decoded) => {
      ...
      // A call which names a tool in the toolkit belongs to that tool, and
