      ...
      if (options.unordered) {
        ...
        yield* semaphore.take(1).pipe(
          ...
          Effect.flatMap((value) => {
            const index = i++
            // Call `f` inside the fiber, so a synchronous throw is that fiber's defect
...
// Runs `self` again each time it completes, until `onDone` ends the loop with
// a `Cause.Done`. Each run gets a scope forked from `scope`, closed before the
// next run starts. The runs are a loop over one pull, rather than a nesting of
// `concatWith`, so the work and memory per run stay constant.
