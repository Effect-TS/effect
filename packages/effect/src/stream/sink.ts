...
export interface Sink<E, S, A, B> {
  readonly initial: T.Effect<T.NoEnv, E, SinkStep<A, S>>;
  step(state: S, next: A): T.Effect<T.NoEnv, E, SinkStep<A, S>>;
  extract(step: S): T.Effect<T.NoEnv, E, B>;
}
 ...
 * @param multi
 */
export function stepMany<E, S, A, B>(
  sink: Sink<E, S, A, B>,
  s: S,
  multi: readonly A[]
): T.Effect<T.NoEnv, E, SinkStep<A, S>> {
  function go(
...
export function collectArraySink<E, A>(): Sink<E, A[], A, A[]> {
  ...
  function step(
    state: A[],
    next: A
  ): T.Effect<T.NoEnv, E, SinkStep<never, A[]>> {
...
export function drainSink<E, A>(): Sink<E, void, A, void> {
  ...
  function step(
    _state: void,
    _next: A
  ): T.Effect<T.NoEnv, E, SinkStep<never, void>> {
...
export function constSink<E, A, B>(b: B): Sink<E, void, A, B> {
  ...
  function step(
    _state: void,
    _next: A
  ): T.Effect<T.NoEnv, E, SinkStep<any, void>> {
...
export function headSink<E, A>(): Sink<E, Option<A>, A, Option<A>> {
  ...
  function step(
    _state: Option<A>,
    next: A
  ): T.Effect<T.NoEnv, E, SinkStep<any, Option<A>>> {
...
export function lastSink<E, A>(): Sink<E, Option<A>, A, Option<A>> {
  ...
  function step(
    _state: Option<A>,
    next: A
  ): T.Effect<T.NoEnv, E, SinkStep<never, Option<A>>> {
 ...
 * @param f
 */
export function evalSink<E, A>(
  f: FunctionN<[A], T.Effect<T.NoEnv, E, unknown>>
): Sink<E, void, A, void> {
  ...
  function step(
    ...
    return pipe(
      f(next),
      T.apSecond(
        T.right(sinkCont(_state)) as T.Effect<T.NoEnv, E, SinkStep<never, void>>
      )
    );
  }
 ...
 * @param f
 */
export function drainWhileSink<E, A>(
  f: Predicate<A>
): Sink<E, Option<A>, A, Option<A>> {
 ...
 * @param queue
 */
export function queueSink<E, A>(
  queue: ConcurrentQueue<A>
): Sink<E, void, A, void> {
  const initial = T.right(sinkCont(undefined));

  function step(_state: void, a: A): T.Effect<T.NoEnv, E, SinkStep<A, void>> {
 ...
 * @param queue
 */
export function queueOptionSink<E, A>(
