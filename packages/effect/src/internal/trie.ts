...
const hashNode = <V>(node: Node<V>, prefix: number): number => {
  ...
  let h = node.value === undefined ? 0 : Hash.combine(here, Hash.hash(node.value.value))
...
const TrieProto: TR.Trie<unknown> = {
  ...
  [Hash.symbol]<V>(this: TrieImpl<V>): number {
    ...
    // instead of rebuilding and rehashing every key string.
    return Hash.optimize(TrieSeed ^ (this._root === undefined ? 0 : hashNode(this._root, keySeed)))
  },
