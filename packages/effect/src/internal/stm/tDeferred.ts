...
/** @internal */
export const _await = <E, A>(self: TDeferred.TDeferred<E, A>): STM.STM<A, E> =>
  stm.flatten(
...
export const done = dual<
  <E, A>(either: Either.Either<E, A>) => (self: TDeferred.TDeferred<E, A>) => STM.STM<boolean>,
  <E, A>(self: TDeferred.TDeferred<E, A>, either: Either.Either<E, A>) => STM.STM<boolean>
>(2, (self, either) =>
  core.flatMap(
    tRef.get(self.ref),
    Option.match({
...
export const fail = dual<
  <E>(error: E) => <A>(self: TDeferred.TDeferred<E, A>) => STM.STM<boolean>,
  <E, A>(self: TDeferred.TDeferred<E, A>, error: E) => STM.STM<boolean>
>(2, (self, error) => done(self, Either.left(error)))

/** @internal */
export const make = <E, A>(): STM.STM<TDeferred.TDeferred<E, A>> =>
  core.map(
...
export const succeed = dual<
  <A>(value: A) => <E>(self: TDeferred.TDeferred<E, A>) => STM.STM<boolean>,
  <E, A>(self: TDeferred.TDeferred<E, A>, value: A) => STM.STM<boolean>
