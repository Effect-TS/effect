...
export interface ConcurrentRef<A> {
   ...
   */
  readonly get: T.Effect<T.AsyncContext, never, A>;
  /**
   * Set the current value of the ConcurrentRef
   * @param a
   */
  set<R>(a: T.Effect<R, never, A>): T.Effect<T.AsyncContext & R, never, A>;
  /**
   * Update the current value of the ConcurrentRef with an effect.
   * Produces the new value
   * @param f
   */
  update<R>(f: F.FunctionN<[A], T.Effect<R, never, A>>): T.Effect<T.AsyncContext & R, never, A>;
  /**
  ...
  ): T.Effect<T.AsyncContext & R, never, B>;
}
...
export const makeConcurrentRef = <A>(initial: A): T.Effect<T.NoEnv, never, ConcurrentRef<A>> =>
  T.effect.map(makeSemaphore(1), (semaphore) => {
    ...
    const set = <R>(a: T.Effect<R, never, A>): T.Effect<T.AsyncContext & R, never, A> =>
    ...
    ): T.Effect<T.AsyncContext & R, never, A> =>
      semaphore.withPermit(
        T.effect.map(
          T.effect.chain(
            T.sync(() => value),
