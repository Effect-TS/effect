  ...
  return toM(
    (r) =>
      ({
        ...
        suspended: T.effect.map(T.provide(r)(suspended), (m) => (_: unknown) =>
          fromM(m)(r)
        )
      } as any)
  )
...
) => (wa: Op.Option<A>) => Managed<S, R, E, Separated<Op.Option<B>, Op.Option<C>>> = (
  f
) => (wa) => Op.option.wilt(managed)(wa, f)
