...
export interface ConcurrentQueue<A> {
  readonly take: T.Stack<T.NoEnv, never, A>;
  offer(a: A): T.Stack<T.NoEnv, never, void>;
}
...
function makeConcurrentQueueImpl<A>(
  state: Ref<State<A>>,
  factory: T.Stack<T.NoEnv, never, Deferred<T.NoEnv, never, A>>,
  ...
  offerGate: T.Stack<T.NoEnv, never, void>,
  ...
  takeGate: FunctionN<[T.Stack<T.NoEnv, never, A>], T.Stack<T.NoEnv, never, A>>
): ConcurrentQueue<A> {
  ...
  const offer = (a: A): T.Stack<T.NoEnv, never, void> =>
    T.applySecond(
      offerGate,
      T.uninterruptible(
        T.flatten(
 ...
 */
export function unboundedQueue<A>(): T.Stack<
  T.NoEnv,
  never,
  ConcurrentQueue<A>
> {
...
export function slidingQueue<A>(
  capacity: number
): T.Stack<T.NoEnv, never, ConcurrentQueue<A>> {
