...
export interface Deferred<R, E, A> {
   ...
   */
  readonly wait: T.Stack<R, E, A>;
  /**
   * Interrupt any waitersa on this Deferred
   */
  interrupt: T.Stack<T.NoEnv, T.NoErr, void>;
  /**
   ...
   * @param a
   */
  done(a: A): T.Stack<T.NoEnv, T.NoErr, void>;
  /**
   *
   ...
   */
  error(e: E): T.Stack<T.NoEnv, T.NoErr, void>;
   ...
   * @param e
   */
  abort(e: unknown): T.Stack<T.NoEnv, T.NoErr, void>;

  /**
   * Complete this deferred with the given cuase
   * @param c
   */
  cause(c: Cause<E>): T.Stack<T.NoEnv, T.NoErr, void>;

  /**
   * complete this Defered with the provide exit status
   * @param e
   */
  complete(e: Exit<E, A>): T.Stack<T.NoEnv, T.NoErr, void>;

  /**
   * Set this deferred with the result of source
   * @param source
   */
  from(source: T.Stack<R, E, A>): T.Stack<T.NoEnv, T.NoErr, void>;
}

/* tested in wave */
/* istanbul ignore next */
export function makeDeferred<R, E, A, E2 = never>(): T.Stack<
  R,
  E2,
  Deferred<R, E, A>
> {
  return T.accessM((r: R) =>
    T.sync(() => {
      const c: Completable<T.Stack<R, E, A>> = completable();
      const wait: T.Stack<R, E, A> = T.flatten(
        T.asyncTotal<T.Stack<R, E, A>>(callback => c.listen(callback)) as any // TODO: this is fine, typedoc thinks differently
      );

      const interrupt: T.Stack<T.NoEnv, T.NoErr, void> = T.sync(() => {
        c.complete(T.raiseInterrupt);
      });

      const done = (a: A): T.Stack<T.NoEnv, T.NoErr, void> =>
        T.sync(() => {
          c.complete(T.pure(a));
        });

      const error = (e: E): T.Stack<T.NoEnv, T.NoErr, void> =>
        T.sync(() => {
          c.complete(T.raiseError(e));
        });

      const abort = (e: unknown): T.Stack<T.NoEnv, T.NoErr, void> =>
        T.sync(() => {
          c.complete(T.raiseAbort(e));
        });

      const cause = (e: Cause<E>): T.Stack<T.NoEnv, T.NoErr, void> =>
        T.sync(() => {
          c.complete(T.raised(e));
        });

      const complete = (exit: Exit<E, A>): T.Stack<T.NoEnv, T.NoErr, void> =>
        T.sync(() => {
