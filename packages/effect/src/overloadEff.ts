...
export type EffypeOf<X> = X extends GenEffect<infer S, infer R, infer E, infer A> ? R : never;
...
export interface PipeableMonadThrow4EC<F extends EffURI, E> {
  readonly fromOption: (onNone: () => E) => <A>(ma: Option<A>) => Kind4<F, never, NoEnv, E, A>;
  ...
  readonly fromPredicate: {
    ...
    <A>(predicate: Predicate<A>, onFalse: (a: A) => E): (a: A) => Kind4<F, never, NoEnv, E, A>;
  };
