...
against `z.compile(schema, { strict: true })`. The fixture families name the
execution mode. Cases cover simple and nested Structs, Arrays, tuples, Records,
anyOf/oneOf Unions, transformations, middleware, recursive schemas,
Declarations, construction, and successful and failing validation.
...
because Zod has no separate constructor operation.
