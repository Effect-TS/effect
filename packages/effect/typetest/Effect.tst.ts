...
class TrackMappedValue extends Context.Service<TrackMappedValue, { readonly value: number }>()(
  "TrackMappedValue"
) {}
...
describe("Effect.track", () => {
  ...
  const source: Effect.Effect<number, "negative", TrackMappedValue> = Effect.gen(function*() {
    const { value } = yield* TrackMappedValue
  ...
  const stringInput = (exit: Exit.Exit<number, string>): number => Exit.isSuccess(exit) ? exit.value : 0
  ...
  it("preserves narrower actual errors and required services", () => {
    expect(observe(source)).type.toBe<Effect.Effect<number, "negative", TrackMappedValue>>()
    expect(source.pipe(observe)).type.toBe<Effect.Effect<number, "negative", TrackMappedValue>>()
    expect(source.pipe(Effect.track(gauge, stringInput))).type.toBe<
      Effect.Effect<number, "negative", TrackMappedValue>
    >()
  })
  ...
  it("preserves both old explicit generic arities", () => {
    ...
    expect(explicit<"negative", TrackMappedValue>(source)).type.toBe<
      Effect.Effect<number, "negative", TrackMappedValue>
    >()
  ...
  it("accepts broad unknown and union mappers without widening source errors", () => {
    ...
    expect(broad(source)).type.toBe<Effect.Effect<number, "negative", TrackMappedValue>>()
    expect(broad(numberFailure)).type.toBe<Effect.Effect<number, number>>()
    expect(union(source)).type.toBe<Effect.Effect<number, "negative", TrackMappedValue>>()
  ...
  it("supports a generic source wrapper within the mapper domain", () => {
    ...
    expect(wrap(source)).type.toBe<Effect.Effect<number, "negative", TrackMappedValue>>()
  })
