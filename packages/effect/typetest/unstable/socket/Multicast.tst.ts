...
describe("Multicast", () => {
  ...
  it("requires IPv6 membership interfaces and keeps interface families paired with groups", () => {
    ...
    expect("192.0.2.1").type.not.toBeAssignableTo<Multicast.NetworkInterface>()
    expect(1).type.not.toBeAssignableTo<Multicast.NetworkInterface>()
    expect(NetAddress.ipv4Loopback).type.not.toBeAssignableTo<Multicast.NetworkInterface>()
    expect(interfaceV4).type.toBeAssignableTo<Multicast.NetworkInterface>()
    expect(interfaceV6).type.toBeAssignableTo<Multicast.NetworkInterface>()
  })
})
