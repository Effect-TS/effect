...
describe("Layer", () => {
  ...
  describe("mock", () => {
    it("does not invent a thenable for a mocked service without a then property", async () => {
      const Service = Context.Service<{ read(): Effect.Effect<number> }>("mock/NoInventedThen")
      const layer = Layer.mock(Service, { read: () => Effect.succeed(42) })
      const exit = await Effect.runPromiseExit(Service.pipe(Effect.provide(layer)))
      assert.strictEqual(exit._tag, "Success")
      if (Exit.isSuccess(exit)) {
        assert.isUndefined(Reflect.get(exit.value, "then"))
      }
      let resolved = false
      void Effect.runPromise(Service.pipe(Effect.provide(layer))).then(() => {
        resolved = true
      }, () => {
        resolved = true
      })
      await setImmediate()
      assert.isTrue(resolved, "runPromise must resolve for a mocked service without a then property")
    })
