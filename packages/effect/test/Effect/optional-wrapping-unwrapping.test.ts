    ...
    describe("Some", () => {
      it.effect("Success", () =>
        Effect.gen(function*() {
          ...
          const resultDataFirstWithOptionSome = yield* Effect.transposeMapOption(
            Option.some(42),
          ...
          const resultDataFirstWithOptionNone = yield* Effect.transposeMapOption(
            Option.some(42),
            () => Effect.succeedNone
          )
          ...
          const resultDataLastWithOptionNone = yield* pipe(
            Option.some(42),
            Effect.transposeMapOption(() => Effect.succeedNone)
          )
