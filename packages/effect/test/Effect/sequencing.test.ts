...
describe("Effect", () => {
  ...
  it.effect("tap", () =>
    Effect.gen(function*() {
      ...
      const a3 = Effect.tap(Effect.succeed(0), (n) => Effect.succeed(n + 1), { onlyEffect: true })
      const a4 = Effect.succeed(0).pipe(Effect.tap((n) => Effect.succeed(n + 1)))
      const a5 = Effect.succeed(0).pipe(Effect.tap((n) => Effect.succeed(n + 1), { onlyEffect: true }))
      const a6 = Effect.succeed(0).pipe(Effect.tap("ok"))
      const a7 = Effect.succeed(0).pipe(Effect.tap(() => "ok"))
      const a8 = Effect.tap(Effect.succeed(0), () => "ok")
      const a9 = Effect.tap(Effect.succeed(0), "ok")
      const a10 = Effect.tap(Effect.succeed(0), () => Promise.resolve("ok"))
