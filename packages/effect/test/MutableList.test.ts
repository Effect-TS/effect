...
describe("MutableList", () => {
  ...
  it("keeps the next element after takeN ends at a bucket boundary", () => {
    const single = MutableList.make<number>()
    MutableList.append(single, 1)
    MutableList.appendAll(single, [2])
    deepStrictEqual(MutableList.takeN(single, 1), [1])
    strictEqual(MutableList.take(single), 2)
    strictEqual(single.length, 0)
    MutableList.append(single, 99)
    strictEqual(MutableList.take(single), 99)
    strictEqual(single.length, 0)

    const partial = MutableList.make<number>()
    MutableList.appendAll(partial, [1, 2, 3])
    MutableList.appendAll(partial, [4])
    deepStrictEqual(MutableList.takeN(partial, 2), [1, 2])
    deepStrictEqual(MutableList.takeAll(partial), [3, 4])
