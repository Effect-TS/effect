...
describe("ScopedCache", () => {
  ...
  describe("lookup ownership", () => {
    ...
    it.effect("a dying finalizer while replacing an expired entry does not strand the key", () =>
      Effect.gen(function*() {
        let lookups = 0
        const cache = yield* ScopedCache.makeWith({
          capacity: 10,
          lookup: (k: number) =>
            Effect.acquireRelease(Effect.sync(() => `${k}:${++lookups}`), () => Effect.die("finalizer-boom")),
          timeToLive: () => Duration.millis(1)
        })
        const first = yield* Effect.exit(ScopedCache.get(cache, 1))
        yield* TestClock.adjust(Duration.millis(10))
        // The expired entry's scope is closed on the miss path, and its
        // finalizer dies. The caller sees the defect; the key must not be left
        // holding an entry that never completes.
        const second = yield* Effect.forkChild(ScopedCache.get(cache, 1), { startImmediately: true })
        yield* settle
        const third = yield* Effect.forkChild(ScopedCache.get(cache, 1), { startImmediately: true })
        yield* settle
        assert.deepStrictEqual(
          {
            first: show(first),
            second: show(second.pollUnsafe()),
            third: show(third.pollUnsafe()),
            lookups
          },
          {
            first: "ok:1:1",
            second: "die:finalizer-boom",
            third: "die:finalizer-boom",
            lookups: 1
          }
        )
      }))

    it.effect("interrupting the caller that is closing an expired entry leaves no entry behind", () =>
      Effect.gen(function*() {
        ...
        const cache = yield* ScopedCache.makeWith({
          capacity: 10,
          lookup: (_: number) =>
            Effect.acquireRelease(Effect.sync(() => ++lookups), (n) =>
              n === 1
                ? Effect.andThen(Deferred.await(gate), Effect.sync(() => released.push(n)))
                : Effect.sync(() => released.push(n))),
          timeToLive: () => Duration.millis(1)
        })
        ...
        const caller = yield* Effect.forkChild(ScopedCache.get(cache, 1), { startImmediately: true })
        yield* settle
        const interrupter = yield* Effect.forkChild(Fiber.interrupt(caller), { startImmediately: true })
        yield* settle
        ...
        assert.deepStrictEqual(
          {
            caller: show(caller.pollUnsafe()),
            interrupter: interrupter.pollUnsafe() !== undefined,
            released,
            later: show(later),
            lookups
          },
          { caller: "interrupted", interrupter: true, released: [1], later: "ok:2", lookups: 2 }
        )
      }))

    it.effect("a caller arriving while an expired entry is being closed shares the new lookup", () =>
      Effect.gen(function*() {
        const gate = yield* Deferred.make<void>()
        let lookups = 0
        const cache = yield* ScopedCache.makeWith({
          ...
          lookup: (_: number) =>
            Effect.acquireRelease(
              Effect.sync(() => ++lookups),
              (n) => n === 1 ? Effect.asVoid(Deferred.await(gate)) : Effect.void
            ),
          timeToLive: () => Duration.millis(1)
        })
        assert.strictEqual(yield* ScopedCache.get(cache, 1), 1)
        ...
        yield* settle
        assert.deepStrictEqual(
          { first: show(first.pollUnsafe()), second: show(second.pollUnsafe()), lookups },
          { first: "ok:2", second: "ok:2", lookups: 2 }
        )
      }))
    ...
    it.effect("the last caller leaving while its lookup completes releases the resource", () =>
      Effect.gen(function*() {
        const failed = yield* atEveryBudget((budget) =>
          Effect.gen(function*() {
            const gate = yield* Deferred.make<void>()
            ...
            const cache = yield* ScopedCache.make({
              ...
              lookup: (_: number) =>
                Effect.acquireRelease(Effect.sync(() => ++acquired), () => Effect.sync(() => released++)).pipe(
                  Effect.flatMap((n) => Effect.as(Deferred.await(gate), n))
                )
            })
            const caller = yield* Effect.forkChild(budget(ScopedCache.get(cache, 1)), { startImmediately: true })
            yield* settle
            // The lookup finishes and the only caller is interrupted at the
            // same time: whichever wins, a resource that is not cached must be
            // released.
            yield* Deferred.succeed(gate, undefined)
            yield* Effect.forkChild(Fiber.interrupt(caller), { startImmediately: true })
            yield* settle
            return acquired - released === (yield* ScopedCache.size(cache))
          })
        )
        assert.deepStrictEqual(failed, [])
      }), 60000)
