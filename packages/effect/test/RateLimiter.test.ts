  ...
  describe.concurrent("fixed-window", () => {
    ...
    it.scoped("will use the provided cost", () =>
      Effect.gen(function*(_) {
        ...
        const fib = yield* Effect.replicateEffect(rl(Clock.currentTimeMillis).pipe(RateLimiter.withCost(10)), 20).pipe(
          Effect.fork
        )
    ...
    it.scoped("will respect different costs per effect and interleave them.", () =>
      Effect.gen(function*(_) {
        ...
        const fib = yield* Effect.all(tasks, { concurrency: "unbounded" }).pipe(Effect.fork)
    ...
    it.scoped("will be composable with other `RateLimiters`", () =>
      Effect.gen(function*(_) {
        ...
        const fib = yield* Effect.replicateEffect(rl(Clock.currentTimeMillis), 32).pipe(Effect.fork)

        const timestamps = yield* Effect.replicateEffect(
  ...
  describe.concurrent("token-bucket", () => {
    ...
    it.scoped("uses the token-bucket algorithm for token replenishment", () =>
      Effect.scoped(Effect.gen(function*(_) {
        ...
        assert.isFalse(yield* Deferred.isDone(deferred))
