...
describe.concurrent("TSubscriptionRef", () => {
  ...
  it.effect("multiple subscribers can receive committed values", () =>
    Effect.gen(function*($) {
      ...
      const subscriber1 = yield* $(pipe(
        ...
        Stream.take(3),
        Stream.tap((v) => Effect.sync(() => console.log(`received value 1: ${v}`))),
        Stream.runCollect,
        Effect.fork
      ))
      ...
      const subscriber2 = yield* $(pipe(
        ...
        Stream.take(2),
        Stream.tap((v) => Effect.sync(() => console.log(`received value 2: ${v}`))),
        Stream.runCollect,
        Effect.fork
      ))
  ...
  it.effect("concurrent subscribes and unsubscribes are handled correctly", () =>
    Effect.gen(function*($) {
      const subscriber = (TSubscriptionRef: TSubscriptionRef.TSubscriptionRef<number>) =>
        pipe(
          Random.nextIntBetween(0, 200),
          Effect.flatMap((n) =>
            pipe(
              TSubscriptionRef.changes,
              Stream.take(n),
              Stream.runCollect
            )
          )
