...
describe("Metric registry isolation", () => {
  it.effect.each([
    { name: "shared counter A to B to A", mode: "shared" },
    { name: "constructor attributes remain registry scoped", mode: "constructor" },
    { name: "constant input wrapper forwards registry", mode: "constant" },
    { name: "contextual attributes isolate registries", mode: "contextual" },
    { name: "attribute wrapper isolates registries", mode: "attributes" },
    { name: "different objects isolate different registries", mode: "separate" },
    { name: "different objects aggregate in the same registry", mode: "same-registry-separate" },
    { name: "same object aggregates in the same registry", mode: "same-registry-shared" }
  ])("$name", ({ mode }) =>
    Effect.gen(function*() {
      ...
      const expected = sameRegistry
        ? [1, 1, 11, 11, 13, 13, 13]
        : mode === "constant"
        ? [1, 0, 1, 1, 2, 1, 2]
        : [1, 0, 10, 1, 3, 10, 3]
      const attributes = ["constructor", "contextual", "attributes"].includes(mode) ? labels : undefined
      assert.deepStrictEqual(observations, expected.map((count) => counterObservation(count, attributes)))
