...
describe("Pool", () => {
  ...
  for (const method of ["get", "use"] as const) {
    it.effect(`retries a failed background acquisition via ${method}`, () =>
      Effect.gen(function*() {
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function*() {
          ...
          yield* waitUntil(() => pool.state.waiters.size === borrowers)
