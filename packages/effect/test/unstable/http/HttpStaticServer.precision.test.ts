...
import { HttpPlatform, HttpServerRequest, HttpServerResponse, HttpStaticServer } from "effect/unstable/http"

type FileOptions = Parameters<HttpPlatform.HttpPlatform["Service"]["fileResponse"]>[1]

const services = (size: bigint, calls: Array<{ path: string; options: FileOptions }>) =>
  Layer.mergeAll(
    Path.layer,
    ...
    for (const url of ["/file.bin", "/directory"]) {
      for (
        ...
        it.effect(`preserves ${range} and the exact ${size}-byte total for ${url}`, () => {
          ...
          }).pipe(Effect.provide(services(size, calls)))
        })
      }
