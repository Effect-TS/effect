...
describe("LanguageModel", () => {
  ...
  describe("invalid tool calls", () => {
    ...
    it.effect("still fails invalid parameters for a known tool when resolution is disabled", () =>
      Effect.gen(function*() {
        // The option governs unknown tools only. A call which names a tool in
        // the toolkit belongs to that tool, and with resolution disabled the
        // caller owns it, so neither opt-in changes what happens to it
        const error = yield* LanguageModel.generateText({
          prompt: [],
          toolkit: ReturnModeToolkit,
          disableToolCallResolution: true,
    ...
    it.effect("still fails invalid parameters for a known tool in a stream with resolution disabled", () =>
      Effect.gen(function*() {
        const error = yield* LanguageModel.streamText({
          prompt: [],
          toolkit: ReturnModeToolkit,
    ...
    it.effect("leaves no tool call unanswered in the resulting history", () =>
      Effect.gen(function*() {
        ...
        // And a result with no call is rejected just the same, so recovery
        // must never drop the call it answers
        deepStrictEqual(resultIds.filter((id) => !callIds.includes(id)), [])
      }))
