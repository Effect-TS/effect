...
describe("code point checks", () => {
  ...
  it("counts code points at inclusive minimum and maximum boundaries", () => {
    for (let bound = 0; bound <= 8; bound++) {
      ...
      const exact = Schema.is(Schema.String.check(Schema.isCodePointsBetween(bound, bound)))
  ...
  it("checks inclusive ranges and rejects inverted ranges", () => {
    const between = Schema.is(Schema.String.check(Schema.isCodePointsBetween(1, 3)))
    const inverted = Schema.is(Schema.String.check(Schema.isCodePointsBetween(3, 1)))
    for (const [value, count] of cases) {
  ...
  it("rounds down bounds and clamps negative bounds to zero", () => {
    ...
    assert.isTrue(Schema.is(Schema.String.check(Schema.isCodePointsBetween(1.9, 2.9)))("😀a"))
    ...
    assert.isTrue(Schema.is(Schema.String.check(Schema.isCodePointsBetween(-2, -1)))(""))
  })
  ...
  it("validates decoding and encoding with code point error messages", async () => {
    for (
      const [check, invalid, message] of [
        ...
        [Schema.isCodePointsBetween(1, 2), "😀ab", "Expected a string with between 1 and 2 code points"],
        [Schema.isCodePointsBetween(1, 1), "😀a", "Expected a string with 1 code points"]
      ] as const
    ) {
