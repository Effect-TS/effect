...
describe("SchemaRepresentation.toCodeDocument annotations", () => {
  ...
  it("skips an Array subclass annotation", () => {
    class Values extends Array<number> {}
    const output = SchemaRepresentation.toCodeDocument({
      representations: [{
        _tag: "String",
        annotations: { invalid: new Values(1), valid: true },
        checks: []
      }],
      references: {}
    })

    assert.strictEqual(output.codes[0].runtime, `Schema.String.annotate({ "valid": true })`)
  })

  it("skips an Array annotation with an accessor element without evaluating it", () => {
    let reads = 0
    const invalid = [0]
    ...
    const output = SchemaRepresentation.toCodeDocument({
      representations: [{
        _tag: "String",
        annotations: { invalid, valid: true },
        checks: []
      }],
      references: {}
    })
