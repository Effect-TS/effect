...
Ensure a supported `@rstest/core` version is installed (`^0.11.0`), then add the package as a dev dependency:
...
- [API reference](https://effect.website/docs/v4/api/rstest)
...
- **Type namespaces**: the Effect helper namespace is still called `Vitest` (`Vitest.Methods`, `Vitest.Tester`, ...) so test files can move between the two packages without changing their type imports. The `Rstest` type from `@rstest/core` is re-exported without being shadowed.
...
- **`{ fails: true }` in test options**: Rstest only honours `fails` through the `it.fails` modifier, so `@effect/rstest` applies it for you when passed in the options object (`Vitest.TestOptions`).
