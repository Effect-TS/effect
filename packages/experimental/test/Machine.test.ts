...
import { Cause, Chunk, Context, Deferred, Effect, Exit, Layer, pipe, Stream } from "effect"
...
describe("Machine", () => {
  test("counter", () =>
    Effect.gen(function*(_) {
      ...
      const cause = yield* pipe(booted.join, Effect.sandbox, Effect.flip)
  ...
  test("forkWithState", () =>
    Effect.gen(function*(_) {
      ...
      assert.deepStrictEqual(
        // @ts-expect-error
        yield* pipe(booted.send(new IncrementBy({ number: 2 })), Effect.exit),
