...
> = Effect.fnUntraced(function*<Rpcs extends Rpc.Any>(
  ...
  type ClientEntry = {
    ...
    readonly resume: (_: Exit.Exit<any, any>) => void
  } | {
      ...
      return options.onFromClient({
        ...
        Effect.zipRight(
          Effect.async<any, any>((resume) => {
            entries.set(id, { _tag: "Effect", rpc, resume })
            return Effect.suspend(() => {
              entries.delete(id)
              return options.onFromClient({ _tag: "Interrupt", requestId: id })
            })
          })
        )
      )
    ...
    switch (message._tag) {
      case "Chunk": {
        ...
        return entry.mailbox.offerAll(message.values).pipe(
          supportsAck ? Effect.zipRight(options.onFromClient({ _tag: "Ack", requestId: message.requestId })) : identity,
...
export const make: <Rpcs extends Rpc.Any>(
  ...
  RpcClient<Rpcs>,
  never,
  Protocol | Rpc.Context<Rpcs> | Scope.Scope
> = Effect.fnUntraced(function*<Rpcs extends Rpc.Any>(
  group: RpcGroup.RpcGroup<Rpcs>,
  options?: {
    readonly spanPrefix?: string | undefined
  ...
  const { responses, send, supportsAck, supportsTransferables } = yield* Protocol

  type ClientEntry = {
    readonly rpc: Rpc.AnyWithProps
    readonly decodeChunk:
      | ((chunk: ReadonlyArray<unknown>) => Effect.Effect<ReadonlyArray<any>, ParseError, unknown>)
      | undefined
  }
  const entries = new Map<RequestId, ClientEntry>()
      ...
      switch (message._tag) {
        case "Request": {
          ...
          const entry: ClientEntry = {
            rpc,
            decodeChunk: Option.isSome(schemas) ? Schema.decodeUnknown(Schema.Array(schemas.value.success)) : undefined
          }
  ...
  yield* Effect.gen(function*() {
    while (true) {
      const [chunk] = yield* responses.takeAll
      for (const message of chunk) {
        switch (message._tag) {
          case "Chunk": {
            const requestId = parseRequestId(message.requestId)
            const entry = entries.get(requestId)
            if (entry && entry.decodeChunk) {
              yield* entry.decodeChunk(message.values).pipe(
          ...
          case "Exit": {
            ...
            if (entry) {
              entries.delete(requestId)
              yield* Schema.decode(getExitSchema(entry.rpc))(message.exit).pipe(
                Effect.locally(FiberRef.currentContext, context),
                Effect.orDie,
          ...
          case "Defect": {
            yield* write({ _tag: "Defect", clientId: 0, defect: decodeDefect(message.defect) })
            break
          }
        }
      }
...
export class Protocol extends Context.Tag("@effect/rpc/RpcClient/Protocol")<Protocol, {
  readonly responses: Mailbox.ReadonlyMailbox<FromServerEncoded>
  readonly send: (
 ...
 * @category protocol
 */
export const makeProtocolHttp: <E, R>(
  client: HttpClient.HttpClient.With<E, R>
) => Effect.Effect<
  Protocol["Type"],
  never,
  RpcSerialization.RpcSerialization | Scope.Scope | R
> = Effect.fnUntraced(function*<E, R>(client: HttpClient.HttpClient.With<E, R>) {
  ...
  const send = (request: FromClientEncoded): Effect.Effect<void> => {
    ...
    return client.post("/", { body }).pipe(
      Effect.provide(context as Context.Context<R>),
      Effect.flatMap((r) =>
...
export const makeProtocolSocket = Effect.gen(function*() {
  ...
  yield* Effect.suspend(() => {
    ...
    return socket.runRaw((message) => {
      try {
        responses.unsafeOfferAll(parser.decode(message) as any)
      } catch (defect) {
        responses.unsafeOffer({ _tag: "Defect", defect })
      }
    })
  }).pipe(
    Effect.tapErrorCause((cause) => responses.offer({ _tag: "Defect", defect: Cause.squash(cause) })),
    ...
    yield* backing.run((message) => {
      ...
      if (response._tag === "Exit") {
        ...
        if (entry) {
          ...
          responses.unsafeOffer(response)
...
const exitSchemaCache = new WeakMap<
  Rpc.AnyWithProps,
  Schema.Schema<Exit.Exit<any, any>, Schema.ExitEncoded<any, any, unknown>>
>()

const getExitSchema = (rpc: Rpc.AnyWithProps) => {
  if (exitSchemaCache.has(rpc)) {
    return exitSchemaCache.get(rpc)!
  }
  const streamSchemas = RpcSchema.getStreamSchemas(rpc.successSchema.ast)
  const schema = Option.isNone(streamSchemas) ?
    Schema.Exit({
    ...
    : Schema.Exit({
      success: Schema.Void,
      failure: Schema.Union(streamSchemas.value.failure, rpc.errorSchema),
      defect: Schema.Defect
    })
  exitSchemaCache.set(rpc, schema as any)
  return schema
}
