 ...
 * @category models
 */
export interface Router<Reqs extends Schema.TaggedProcedure.All, R> extends Pipeable {
...
const fromSet = <Reqs extends Schema.TaggedProcedure.All, R>(
...
export const provideServiceEffect: {
  ...
  ): <Reqs extends Schema.TaggedProcedure.All, R>(self: Router<Reqs, R>) => Router<Reqs, Exclude<R, I> | R2>
  <Reqs extends Schema.TaggedProcedure.All, R, I, S, E, R2>(
    self: Router<Reqs, R>,
    tag: Context.Tag<I, S>,
    effect: Effect.Effect<S, E, R2>
  ): Router<Reqs, Exclude<R, I> | R2>
} = dual(3, <Reqs extends Schema.TaggedProcedure.All, R, I, S, E, R2>(
  self: Router<Reqs, R>,
  tag: Context.Tag<I, S>,
  effect: Effect.Effect<S, E, R2>
...
export const provideService: {
  <I, S>(
    tag: Context.Tag<I, S>,
    service: S
  ): <Reqs extends Schema.TaggedProcedure.All, R>(self: Router<Reqs, R>) => Router<Reqs, Exclude<R, I>>
  <Reqs extends Schema.TaggedProcedure.All, R, I, S>(
    self: Router<Reqs, R>,
    tag: Context.Tag<I, S>,
    service: S
  ): Router<Reqs, Exclude<R, I>>
} = dual(3, <Reqs extends Schema.TaggedProcedure.All, R, I, S>(
  self: Router<Reqs, R>,
  tag: Context.Tag<I, S>,
  service: S
...
export const toHandlerRaw = <R extends Router<any, any>>(router: R) => {
  const schema: Schema.Schema<
    readonly [Schema.TaggedProcedure.All, Rpc.Rpc<any, any>],
    unknown,
    Router.ContextRaw<R>
