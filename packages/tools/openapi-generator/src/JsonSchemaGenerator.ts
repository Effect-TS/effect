  ...
  function makeCodeDocument(
    ...
    if (Arr.isArrayNonEmpty(schemasWithExamples)) {
      ...
      for (let i = 0; i < schemasWithExamples.length; i++) {
        ...
        const isValid = Schema.is(exampleSchemas[i])
        const validExamples = examples.filter((example) => {
          if (isValid(example)) return true
          options?.onWarning?.(makeInvalidExampleWarning(node, example))
          return false
        })
...
function makeInvalidExampleWarning(
  ...
  return {
    ...
    message: types === undefined
      ? `Dropped an example value of type "${getValueType(example)}" because it is incompatible with the schema.`
      : `Dropped an example value of type "${getValueType(example)}" because it is incompatible with schema type ${
        formatTypes(types)
      }.`
  }
}
