...
const generateWithPatch = (patch: string) =>
  Effect.gen(function*() {
    ...
    const output = yield* generator.generate(provider, {
      openapi: "3.1.0",
  ...
  describe("CodeGenerator patch inputs", () => {
    it.effect("accepts inline patches with leading whitespace", () =>
      Effect.gen(function*() {
        ...
        assert.strictEqual(result.info.title, "After")
      }))

    it.effect("accepts absolute paths with leading whitespace", () =>
      Effect.gen(function*() {
        const path = yield* Path.Path
        const patch = path.join(import.meta.dirname, "fixtures/patches/valid-replace.json")
        const result = yield* generateWithPatch(` ${patch}`)
        assert.strictEqual(result.info.title, "Updated API Title")
