 ...
 * `@since` tag into a minimal JSDoc block, and copies `@stability unstable` unless the module
...
const extractModuleMetadata = (file: string, content: string): Effect.Effect<ModuleMetadata, BarrelCodegenError> => {
  ...
  return Effect.succeed({
    since,
    unstable: !isInternalModule(file, block) && /^\s*\*\s*@stability\s+unstable\s*$/m.test(block)
  })
}
