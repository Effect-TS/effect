...
const declarationKind = (node: ts.Node): SemanticModel.DeclarationKind => {
 ...
 *
 * This parser uses only the TypeScript syntax tree and does not create a type checker or
 * require declarations to be exported.
 *
 * @category testing
 * @since 0.6.0
 */
export const extract = (options: {
...
}): ReadonlyArray<SemanticModel.Example> => {
  ...
  const examples: Array<SemanticModel.Example> = []
...
/**
 * Reads a TypeScript source file and extracts all of its JSDoc examples using syntax-only parsing.
 *
 * @category testing
 * @since 0.6.0
 */
export const extractFile = (options: {
  readonly file: string
  readonly packageName: string
  readonly packageRoot: string
  readonly workspaceRoot: string
}): Promise<ReadonlyArray<SemanticModel.Example>> =>
