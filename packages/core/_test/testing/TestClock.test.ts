describe.concurrent("TestClock", () => {
  it("sleep does not require passage of clock time", () =>
    Do(($) => {
    ...
    }).provideLayer(TestEnvironment).unsafeRunPromise())

  it("sleep delays effect until time is adjusted", () =>
    Do(($) => {
      const ref = $(Ref.make(false))
      $(ref.set(true).delay((10).hours).fork)
  ...
  it("sleep correctly handles multiple sleeps", () =>
    Do(($) => {
      const ref = $(Ref.make(""))
  ...
  it("sleep correctly handles new set time", () =>
    Do(($) => {
      const ref = $(Ref.make(false))
      $(ref.set(true).delay((10).hours).fork)
  ...
  it("adjust correctly advances currentTime", () =>
    Do(($) => {
  ...
  it("adjust does not produce sleeps", () =>
    Do(($) => {
      $(TestClock.adjust((1).millis))
  ...
  it("clock time is always 0 at the start of a test that repeats", () =>
    Do(($) => {
  ...
  it("interacts correctly with Scheduled.fixed", () =>
    Do(($) => {
  ...
  it("adjustments to time are visible on other fibers", () =>
    Do(($) => {
  ...
  it("adjustWith runs the specified effect and advances the clock", () =>
    Do(($) => {
