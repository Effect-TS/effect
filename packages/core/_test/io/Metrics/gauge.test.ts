...
describe.concurrent("Metrics", () => {
  describe.concurrent("Gauge", () => {
    it("custom set as aspect", async () => {
      ...
      const program = Effect.succeed(1)(gauge.apply) >
        Effect.succeed(3)(gauge.apply) >
        gauge.value();
    ...
    it("gauge + taggedWith", async () => {
      ...
      const program = Effect.succeed("hello").apply(gauge.apply) >
        Effect.succeed("!").apply(gauge.apply) >
        Effect.succeed("!").apply(gauge.apply) >
