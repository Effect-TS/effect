...
export class Counter {
  ...
  get acquired(): Effect<never, never, number> {
    return this.ref.get.map((tuple) => tuple.get(0))
  }
  ...
  get incrementAcquire(): Effect<never, never, number> {
    return this.ref.modify(({ tuple: [acquire, release] }) =>
      Tuple(acquire + 1, Tuple(acquire + 1, release))
    )
  }

  get incrementRelease(): Effect<never, never, number> {
    return this.ref.modify(({ tuple: [acquire, release] }) =>
      Tuple(release + 1, Tuple(acquire, release + 1))
    )
  }
