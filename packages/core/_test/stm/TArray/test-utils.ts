...
export function makeTArray<A>(n: number, a: A): STM<unknown, never, TArray<A>> {
  return TArray.from(Chunk.fill(n, () => a));
}

export function makeStair(n: number): STM<unknown, never, TArray<number>> {
  return TArray.from(Chunk.range(1, n));
}

export function makeStairWithHoles(n: number): STM<unknown, never, TArray<Option<number>>> {
  return TArray.from(
...
export function valuesOf<A>(tArray: TArray<A>): STM<unknown, never, Chunk<A>> {
