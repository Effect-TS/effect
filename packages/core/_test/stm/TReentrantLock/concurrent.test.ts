describe.concurrent("ReentrantLock", () => {
  ...
  it("2 locks from different fibers", () =>
    Do(($) => {
      ...
      $(
        Effect.scoped(lock.withLockScoped.flatMap((count) => mlatch.succeed(undefined).as(count)))
          .fork
      )
      $(mlatch.await)
      const reader = $(
        Effect.scoped(lock.withLockScoped.flatMap(count => wlatch.succeed(undefined).as(count)))
          .fork
      )
      $(wlatch.await)
  ...
  it("cleans up interrupted waiters", () =>
    Do(($) => {
      ...
      $(
        Effect.scoped(
          lock.withLockScoped
            .zipRight(mlatch.succeed(undefined))
            .zipRight(wlatch.await)
        ).fork
      )
      $(mlatch.await)
      const fiber1 = $(
      ...
      $(
        latch1.countDown.zipRight(
          Effect.scoped(
            lock.withLockScoped.zipRight(
              ref.update((n) => n + 10).zipLeft(wlatch2.succeed(undefined))
            )
          )
        ).fork
      )
  ...
  it("assigns lock to fibers randomly", () =>
    Do(($) => {
      ...
      const program = Do(($) => {
        ...
        const fiber1 = $(
          latch.countDown.zipRight(Effect.scoped(lock.withLockScoped.zipRight(ref.update(f1)))).fork
        )
        const fiber2 = $(
          latch.countDown.zipRight(Effect.scoped(lock.withLockScoped.zipRight(ref.update(f2)))).fork
        )
        const fiber3 = $(
          latch.countDown.zipRight(Effect.scoped(lock.withLockScoped.zipRight(ref.update(f3)))).fork
        )
        const fiber4 = $(
          latch.countDown.zipRight(Effect.scoped(lock.withLockScoped.zipRight(ref.update(f4)))).fork
        )
