  ...
  describe.concurrent("locks", () => {
    ...
    it("1 write lock then 1 read lock, different fibers", async () => {
      const program = Do(($) => {
        ...
        $(
          Effect.scoped(
            lock.writeLock().flatMap((count) => rlatch.succeed(undefined as void) > wlatch.await().as(count))
          ).fork()
        )

        $(rlatch.await())

        const reader = $((mlatch.succeed(undefined as void) > Effect.scoped(lock.readLock())).fork())
        ...
        $(wlatch.succeed(undefined as void))
    ...
    it("1 write lock then 1 write lock, different fibers", async () => {
      const program = Do(($) => {
        ...
        const reader = $((mlatch.succeed(undefined as void) > Effect.scoped(lock.writeLock())).fork())
    ...
    it("upgrade read lock to write lock from same fiber", async () => {
      const program = Do(($) => {
        ...
        $(rlatch.succeed(undefined as void))
