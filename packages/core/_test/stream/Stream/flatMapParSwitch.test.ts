  ...
  describe.concurrent("flatMapParSwitch", () => {
        ...
        .tap(({ lastExecuted, semaphore }) =>
          Stream(1, 2, 3, 4)
            .flatMapParSwitch(1, (i) =>
              i > 3
                ? Stream.acquireReleaseUse(Effect.unit, () => lastExecuted.set(true)).flatMap(() => Stream.empty)
        ...
        .tap(({ lastExecuted, semaphore }) =>
          Stream.range(1, 13)
            .flatMapParSwitch(4, (i) =>
              i > 8
                ? Stream.acquireReleaseUse(Effect.unit, () => lastExecuted.update((n) => n + 1)).flatMap(() =>
                  Stream.empty
                )
                : Stream.scoped(semaphore.withPermitScoped()).flatMap(
                  () => Stream.never
    ...
    it("finalizer ordering", async () => {
      const program = Effect.Do()
        ...
        .bindValue("inner", ({ push }) => Stream.acquireReleaseUse(push("InnerAcquire"), () => push("InnerRelease")))
        .tap(({ inner, push }) =>
          Stream.acquireReleaseUse(push("OuterAcquire").as(inner), () => push("OuterRelease"))
            .flatMapParSwitch(2, identity)
            .runDrain()
        )
