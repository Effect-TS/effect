  ...
  describe.concurrent("flatMap", () => {
    ...
    it("inner finalizers", async () => {
      const program = Effect.Do()
        ...
        .bind("fiber", ({ latch, push }) =>
          Stream(
            Stream.acquireReleaseUse(push(1), () => push(1)),
            Stream.fromEffect(push(2)),
            Stream.acquireReleaseUse(push(3), () => push(3)) >
    ...
    it("finalizer ordering 1", async () => {
      const program = Effect.Do()
        ...
        .bindValue(
          "stream",
          ({ push }) =>
            Stream.acquireReleaseUse(push("open1"), () => push("close1")).flatMap(() =>
              Stream.fromChunks(Chunk(undefined), Chunk(undefined))
                .tap(() => push("use2"))
                .ensuring(push("close2"))
                .flatMap(() =>
                  Stream.acquireReleaseUse(push("open3"), () => push("close3")).flatMap(
                    () =>
                      Stream.fromChunks(Chunk(undefined), Chunk(undefined))
                        .tap(() => push("use4"))
      ...
      const program = Effect.Do()
        ...
        .bindValue("stream", ({ push }) =>
          Stream.fromChunks(Chunk(1), Chunk(2))
            .tap(() => push("use2"))
            .flatMap(() => Stream.acquireReleaseUse(push("open3"), () => push("close3"))))
        .tap(({ stream }) => stream.runDrain())
        .flatMap(({ effects }) => effects.get());
    ...
    it("exit signal", async () => {
      const program = Effect.Do()
        ...
        .bindValue("inner", ({ ref }) =>
          Stream.acquireReleaseExitUse(Effect.unit, (_, exit) =>
            exit.fold(
              () => ref.set(true),
              () => Effect.unit
