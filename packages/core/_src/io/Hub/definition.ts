...
export interface Hub<A> extends Enqueue<A> {
   ...
   * to the hub.
   */
  readonly publish: (a: A, __tsplusTrace?: string) => UIO<boolean>;

  /**
   * Publishes all of the specified messages to the hub, returning whether they
   * were published to the hub.
   */
  readonly publishAll: (as: Collection<A>, __tsplusTrace?: string) => UIO<boolean>;
   ...
   * each time.
   */
  readonly subscribe: Effect<HasScope, never, Dequeue<A>>;
}
