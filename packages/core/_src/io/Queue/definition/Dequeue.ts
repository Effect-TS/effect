...
export interface Dequeue<A> extends CommonQueue {
   ...
   */
  readonly take: Effect<never, never, A>

  /**
   * Removes all the values in the queue and returns the values. If the queue is
   * empty returns an empty collection.
   */
  readonly takeAll: Effect<never, never, Chunk<A>>

  /**
   * Takes up to max number of values from the queue.
   */
  readonly takeUpTo: (max: number, __tsplusTrace?: string) => Effect<never, never, Chunk<A>>
}
