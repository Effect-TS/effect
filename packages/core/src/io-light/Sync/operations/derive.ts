...
/**
 * @tsplus static ets/SyncOps deriveLifted
 */
...
>(
  ...
  return (functions, constants, values) => {
    ...
    for (const k of functions) {
      // @ts-expect-error
      ret[k] = (...args: any[]) => Sync.serviceWithSync(H)((h) => h[k](...args))
    }

    for (const k of constants) {
      // @ts-expect-error
      ret[k] = Sync.serviceWithSync(H)((h) => h[k])
    }

    for (const k of values) {
      ret[k] = Sync.serviceWith(H)((h) => h[k])
    }
...
/**
 * @tsplus static ets/SyncOps deriveAccessSync
 */
export function deriveAccessSync<T>(
  ...
  return (generics) => {
    ...
    for (const k of generics) {
      ret[k] = (f: any) => Sync.serviceWithSync(H)((h) => f(h[k]))
    }
...
/**
 * @tsplus static ets/SyncOps deriveAccess
 */
export function deriveAccess<T>(
  ...
  return (generics) => {
    ...
    for (const k of generics) {
      ret[k] = (f: any) => Sync.serviceWith(H)((h) => f(h[k]))
    }
