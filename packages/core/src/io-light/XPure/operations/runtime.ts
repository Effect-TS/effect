...
import { concrete, XPure } from "../definition"
 ...
 *
 * @ets_data_first runAll_
 */
export function runAll<S1>(s: S1) {
  return <W, S2, E, A>(
    self: XPure<W, S1, S2, unknown, E, A>
  ): Tuple<[Chunk<W>, Either<E, Tuple<[S2, A]>>]> => self.runAll(s)
}
...
export function runState<S1>(s: S1) {
  return <W, S2, A>(self: XPure<W, S1, S2, unknown, never, A>): Tuple<[S2, A]> =>
    self.runState(s)
}
