 ...
 *
 * @tsplus getter effect/core/stm/THub/TDequeue peekOption
 * @category getters
 * @since 1.0.0
 */
export function peekOption<A>(self: THub.TDequeue<A>): STM<never, never, Option.Option<A>> {
  ...
  return STM.Effect((journal, fiberId) => {
    ...
    let a: Option.Option<A> = Option.none
    let loop = true
