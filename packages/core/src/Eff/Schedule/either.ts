 ...
 */
export const either_ = <S, R, ST, A, B, S1, R1, ST1, A1 extends A, C>(
 ...
 * Effects described by this schedule will always be executed before the effects described by the second schedule.
 */
export const either = <A, B, S1, R1, ST1, A1 extends A, C>(
  that: Schedule<S1, R1, ST1, A1, C>
) => <S, R, ST>(
  self: Schedule<S, R, ST, A, B>
 ...
 */
export const eitherWith_ = <S, R, ST, A, B, S1, R1, ST1, A1 extends A, C, D>(
  self: Schedule<S, R, ST, A, B>,
  that: Schedule<S1, R1, ST1, A1, C>,
  f: (b: B, c: C) => D
): Schedule<unknown, R & R1, [ST, ST1], A1, D> =>
  map_(either_(self, that), ([b, c]) => f(b, c))

/**
 * The same as `either` followed by `map`.
 */
export const eitherWith = <B, C, D>(f: (b: B, c: C) => D) => <
  A,
  S1,
  R1,
  ST1,
...
>(
  that: Schedule<S1, R1, ST1, A1, C>
) => <S, R, ST>(self: Schedule<S, R, ST, A, B>) => eitherWith_(self, that, f)
