...
import type { Driver, Schedule } from "../../../io/Schedule"
      ...
      Channel.unwrap(
        schedule()
          ...
          .map((driver) => {
            ...
            > = Channel.readWith(
              (chunk: Chunk<A>) =>
                feed<R, E, A, R2, B, C1, C2>(loop, driver, f, g, chunk),
...
function feed<R, E, A, R2, B, C1, C2>(
  loop: Channel<R & R2 & HasClock, E, Chunk<A>, unknown, E, Chunk<C1 | C2>, void>,
  driver: Driver<unknown, R2, unknown, B>,
  f: (a: A) => C1,
  g: (b: B) => C2,
  input: Chunk<A>,
  __tsplusTrace?: string
): Channel<R & R2 & HasClock, E, Chunk<A>, unknown, E, Chunk<C1 | C2>, void> {
