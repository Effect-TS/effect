...
): Managed<R & R2, E | E2, Tuple<[Z, Stream<unknown, E, A2>]>> {
  return Managed.Do()
    .bind("promise", () => Promise.makeManaged<E | E2, Z>())
    .bind("handoff", () => Handoff.make<Signal<E, A2>>().toManaged())
    .map(({ handoff, promise }) => {
      ...
      return self
        .runManaged(consumer)
        .fork()
        .flatMap(() => promise.await().toManaged())
