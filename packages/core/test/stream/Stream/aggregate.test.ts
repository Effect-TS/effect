...
describe("Stream", () => {
  describe("aggregateAsync", () => {
    ...
    //   expect(List.from(result).flatten().toArray()).toEqual([1, 1, 1, 1])
  ...
  describe("aggregateAsyncWithin", () => {
    ...
    //       Effect.Do()
    //         .bind("fiber", () =>
    //           Stream.fromQueue(c.queue.map((exit) => Take.single(exit)))
    //             .tap(() => c.proceed)
    //             .flattenTake()
    //             .aggregateAsyncWithin(Sink.last(), Schedule.fixed(Duration(100)))
    //             .interruptWhen(Effect.never)
    //             .take(2)
    //             .runCollect()
    //             .fork()
    //         )
    //         .tap(() => (c.offer > c.awaitNext).repeatN(3))
    //         .flatMap(({ fiber }) =>
