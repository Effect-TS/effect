...
import { Config, ConfigProvider, Effect, Layer, pipe, Secret } from "effect"
...
describe("PlatformConfigProvider", () => {
  it.effect("fromFileTree", () =>
    Effect.gen(function*(_) {
      assert.strictEqual(Secret.value(yield* Config.secret("secret")), "keepitsafe")
  ...
  it.effect("layerFileTreeAdd", () =>
    Effect.gen(function*(_) {
      assert.strictEqual(Secret.value(yield* Config.secret("secret")), "shh")
      assert.strictEqual(yield* Config.integer("integer"), 123)
