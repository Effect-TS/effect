...
export const atLeast = dual<
  {
    ...
    (times: number): <A>(self: Args.Args<A>) => Args.Args<Array_.NonEmptyArray<A>>
  },
  {
    <A>(self: Args.Args<A>, times: 0): Args.Args<Array<A>>
    <A>(self: Args.Args<A>, times: number): Args.Args<Array_.NonEmptyArray<A>>
  }
...
export const between = dual<
  {
    ...
    (
      min: number,
      max: number
    ): <A>(self: Args.Args<A>) => Args.Args<Array_.NonEmptyArray<A>>
  },
  {
    ...
    <A>(
      ...
      min: number,
      max: number
    ): Args.Args<Array_.NonEmptyArray<A>>
  }
    ...
    case "Variadic": {
      ...
      > => {
        ...
        return validateInternal(self.args as Instruction, args, config).pipe(Effect.matchEffect({
          onFailure: (failure) =>
            acc.length >= min1 && Array_.isEmptyReadonlyArray(args)
              ? Effect.succeed([args, acc])
              : Effect.fail(failure),
          onSuccess: ([args, a]) => loop(args, Array_.append(acc, a))
        }))
      }
      return loop(args, Array_.empty()).pipe(
    ...
    case "Single": {
      ...
      return InternalPrimitive.wizard(self.primitiveType, help).pipe(
        ...
        Effect.flatMap((input) => {
          const args = Array_.of(input as string)
  ...
  switch (self._tag) {
    ...
    case "Single": {
      ...
      return possibleValues.length === 0
        ? Array_.empty()
        : Array_.of(`${multiple}${optional}${self.name}${description}${possibleValues}`)
    }
    case "Map": {
