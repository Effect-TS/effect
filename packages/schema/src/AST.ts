...
export class TemplateLiteral implements Annotated {
  static make = (
    ...
    Array.isNonEmptyReadonlyArray(spans) ?
...
const sortCandidates = Array.sort(
...
export const flatten = (candidates: ReadonlyArray<AST>): Array<AST> =>
  Array.flatMap(candidates, (ast) => isUnion(ast) ? flatten(ast.types) : [ast])
  ...
  switch (ast._tag) {
    ...
    case "Union":
      return ast.types.slice(1).reduce(
        (out: Array<PropertyKey>, ast) => Array.intersection(out, getPropertyKeys(ast)),
    ...
    switch (ast.transformation._tag) {
      ...
      case "TypeLiteralTransformation": {
        ...
        return new Transformation(
          ...
          pick(ast.to, keys),
          Array.isNonEmptyReadonlyArray(ts) ?
...
const sortPropertySignatures = Array.sort(
  Order.mapInput(Number.Order, (ps: PropertySignature) => getCardinality(ps.type))
)

const sortIndexSignatures = Array.sort(
...
const equalsTemplateLiteralSpan = Array.getEquivalence<TemplateLiteralSpan>((self, that) =>
  self.type._tag === that.type._tag && self.literal === that.literal
)

const equalsEnums = Array.getEquivalence<readonly [string, string | number]>((self, that) =>
...
const intersection = Array.intersectionWith(equals)
