...
export interface Decoder<I, A> extends Schema<A> {
  readonly decode: (options: DecodeOptions) => (i: I) => DE.DecodeResult<A>
}
...
export const decode = <A>(schema: Schema<A>, options: DecodeOptions) =>
  (u: unknown): DE.DecodeResult<A> => decoderFor(schema).decode(options)(u)
...
export const decodeOrThrow = <A>(schema: Schema<A>, options: DecodeOptions) =>
  (u: unknown): A => {
    const t = decoderFor(schema).decode(options)(u)
    if (DE.isFailure(t)) {
  ...
  const go = (ast: AST.AST): Decoder<any, any> => {
    switch (ast._tag) {
            ...
            (input: unknown) => {
              ...
              for (; i < elements.length; i++) {
                if (input.length < i + 1) {
                  ...
                  const t = decoder.decode(options)(input[i])
                  if (DE.isFailure(t)) {
              ...
              if (O.isSome(rest)) {
                ...
                for (; i < input.length - tail.length; i++) {
                  const t = head.decode(options)(input[i])
                  if (DE.isFailure(t)) {
                    return DE.failures(I.mutableAppend(es, DE.index(i, t.left)))
                  } else {
                    if (DE.hasWarnings(t)) {
                ...
                for (let j = 0; j < tail.length; j++) {
                  ...
                  if (input.length < i + 1) {
                    ...
                    const t = tail[j].decode(options)(input[i])
                    if (DE.isFailure(t)) {
                      // the input element is present but is not valid, bail out
                      return DE.failures(I.mutableAppend(es, DE.index(i, t.left)))
                    } else if (DE.hasWarnings(t)) {
          ...
          (options) =>
            (input: unknown) => {
              ...
              for (let i = 0; i < propertySignaturesTypes.length; i++) {
                ...
                const t = decoder.decode(options)(input[name])
                if (DE.isFailure(t)) {
              ...
              if (indexSignatures.length > 0) {
                for (let i = 0; i < indexSignatures.length; i++) {
                  ...
                  for (const key of keys) {
                    ...
                    // handle keys
                    // ---------------------------------------------
                    let t = parameter.decode(options)(key)
                    if (DE.isFailure(t)) {
                    ...
                    // handle values
                    // ---------------------------------------------
                    t = type.decode(options)(input[key])
                    if (DE.isFailure(t)) {
                      return DE.failures(I.mutableAppend(es, DE.key(key, t.left)))
                    } else {
                      if (DE.hasWarnings(t)) {
        ...
        return make(I.makeSchema(ast), (options) =>
          (u) => {
            ...
            for (let i = 0; i < types.length; i++) {
              const t = types[i].decode(options)(u)
              if (DE.isSuccess(t)) {
      ...
      case "Transform": {
        ...
        return make(
          I.makeSchema(ast),
          (options) => (u) => pipe(from.decode(options)(u), I.flatMap((a) => ast.f(options)(a)))
        )
      }
    }
  }
