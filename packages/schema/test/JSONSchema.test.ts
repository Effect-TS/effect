...
describe("JSONSchema", () => {
  ...
  describe("tuple", () => {
    ...
    it("e? + r", () => {
      const schema = S.tupleType([S.optionalElement(S.string)], JsonNumber)
      const jsonSchema = JSONSchema.make(schema)
      expect(jsonSchema).toStrictEqual({
        "$schema": "http://json-schema.org/draft-07/schema#",
        "type": "array",
    ...
    it("r + e should raise an error", () => {
      const schema = S.array(JsonNumber, S.string)
      expect(() => JSONSchema.make(schema)).toThrow(
        new Error(
    ...
    it("e + r", () => {
      const schema = S.tupleType([S.string], JsonNumber)
      const jsonSchema = JSONSchema.make(schema)
      expect(jsonSchema).toStrictEqual({
        "$schema": "http://json-schema.org/draft-07/schema#",
        "type": "array",
  ...
  describe("annotations", () => {
    ...
    it("propertySignatureDeclaration", () => {
      const schema = S.struct({
        foo: S.propertySignatureDeclaration({ schema: S.string }).annotations({
          description: "foo description",
          title: "foo title",
          examples: ["foo example"]
        }),
        bar: S.propertySignatureDeclaration({ schema: JsonNumber }).annotations({
