...
const users = D.pgTable("users", {
  ...
  jsonArray: D.jsonb("json_array").$type<Array<string>>()
})
...
describe.sequential("Pg", () => {
  it.effect("works", () =>
    Effect.gen(function*() {
      ...
      yield* sql`CREATE TABLE users (id SERIAL PRIMARY KEY, name TEXT NOT NULL, snake_case TEXT NOT NULL, json_data JSONB, json_array JSONB)`
      yield* db.insert(users).values({ name: "Alice", snakeCase: "alice" })
      const results = yield* db.select().from(users)
      assert.deepStrictEqual(results, [{ id: 1, name: "Alice", snakeCase: "alice", jsonData: null, jsonArray: null }])
    }).pipe(
  ...
  it.effect("remote callback", () =>
    Effect.gen(function*() {
      ...
      assert.deepStrictEqual(results, [{ id: 1, name: "Alice", snakeCase: "snake", jsonData: null, jsonArray: null }])
    }).pipe(
      Effect.provide(DrizzlePgLive),
      Effect.catchTag("ContainerError", () => Effect.void)
    ), {
  ...
  it.effect("fail properly for conflict issues", () => {
    ...
    return Effect.gen(function*() {
      ...
      yield* sql`CREATE TABLE users (id SERIAL PRIMARY KEY, name TEXT NOT NULL UNIQUE, snake_case TEXT NOT NULL, json_data JSONB, json_array JSONB)`
