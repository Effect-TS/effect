...
describe("storage transaction scheduling", () => {
  ...
  ])("$mode completes after automatic yields with an outside fiber queued", ({ count, mode, success }) =>
    Effect.gen(function*() {
      const bundle = yield* Effect.promise(() =>
        build({
          entryPoints: [fileURLToPath(new URL("./fixtures/transaction.ts", import.meta.url))],
          bundle: true,
          format: "esm",
          platform: "browser",
          write: false
