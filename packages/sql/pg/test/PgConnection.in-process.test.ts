  ...
  it.live.each([
    ...
    ({ applicationName, parameter, urlName, encoding, expected }) =>
      Effect.scoped(Effect.gen(function*() {
        ...
        url.searchParams.set("options", "-capplication_name=options-app")
    ...
    ({ explicit, urlOptions, expected }) =>
      Effect.scoped(Effect.gen(function*() {
        ...
        yield* PgConnection.make({
          url: Redacted.make(url.toString()),
          options: explicit,
          startupParameters: { SEARCH_PATH: "app,public" }
        })
        assert.strictEqual(parameters!.get("search_path"), "app,public")
