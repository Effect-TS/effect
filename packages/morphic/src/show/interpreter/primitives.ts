...
export const showPrimitiveInterpreter = memo(
  <Env extends AnyEnv>(): MatechsAlgebraPrimitive1<ShowURI, Env> => ({
    ...
    bigint: (config) => (env) =>
      new ShowType(
        showApplyConfig(config)({ show: (a) => JSON.stringify(a) }, env, {})
      ),
    ...
    nonEmptyArray: (getShow, config) => (env) =>
      new ShowType(
        showApplyConfig(config)(AgetShow(getShow(env).show), env, {
          show: getShow(env).show
        })
      ),
    uuid: (config) => (env) =>
      new ShowType(showApplyConfig(config)(showString, env, 1)),
    either: (e, a, config) => (env) =>
      new ShowType(
        showApplyConfig(config)(EgetShow(e(env).show, a(env).show), env, {})
      ),
    option: (a, config) => (env) =>
