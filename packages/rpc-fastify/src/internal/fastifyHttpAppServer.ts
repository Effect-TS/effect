...
> =>
  Effect.gen(function*() {
    const handledApp: Effect.Effect<void, never, ServerRequest.HttpServerRequest> = App.toHandled(
      httpApp as App.Default<never, any>,
      handleResponse
    ) as any
...
export const toHandler = <E, R, LE>(
  httpApp: App.Default<E, R>,
  layer: Layer.Layer<Exclude<R, ServerRequest.HttpServerRequest | Scope.Scope>, LE>,
  options?: {
  ...
  const fullLayer = Layer.mergeAll(layer, Layer.scope)
  ...
  function handler(
    ...
    if (!handlerPromise) {
      ...
      // 2. Using that runtime to provide services to toHandlerEffect
      ...
      const buildEffect = Effect.gen(function*() {
        ...
        return yield* (toHandlerEffect(httpApp) as Effect.Effect<Handler, never, any>).pipe(
...
/** @internal */
export class FastifyServerRequest extends Inspectable.Class implements ServerRequest.HttpServerRequest {
