...
const writePackageJsonContent = pipe(
  ...
  TE.tap(({ global }) =>
    global._tag === "None"
      ? TE.unit
      : TE.forEach_(
          global.value.filter((gi) => gi.startsWith("src/")),
...
pipe(
  ...
  TE.tap(() =>
    TE.when(() => fs.existsSync(`./src`))(
      exec(`mkdir -p ./dist/src && cp -r ./src/* ./dist/src`)
    )
  ),
  TE.tap(() =>
    TE.when(() => fs.existsSync(`./build/mjs`))(
      exec(`mkdir -p ./dist/mjs && cp -r ./build/mjs/* ./dist/mjs`)
    )
  ),
  TE.tap(() =>
  ...
  TE.tap(() => exec(`cp -r ./tsconfig.* ./dist`)),
