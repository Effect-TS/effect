...
export interface ReduceRightF<F> extends HasURI<F> {
  ...
  readonly reduce: <A, B>(
    b: B,
    f: (a: A, b: B) => B
  ) => <SI, SO, X, In, S, Env, Err>(fa: HKT8<F, SI, SO, X, In, S, Env, Err, A>) => B
}
...
export interface ReduceRightK<F extends URIS> extends HasURI<F> {
  ...
  readonly reduce: <A, B>(
    b: B,
    f: (a: A, b: B) => B
  ) => <SI, SO, X, In, S, Env, Err>(fa: Kind<F, SI, SO, X, In, S, Env, Err, A>) => B
}
...
export interface ReduceRightFE<F, E> extends HasConstrainedE<F, E> {
  ...
  readonly reduce: <A, B>(
    b: B,
    f: (a: A, b: B) => B
  ) => <SI, SO, X, In, S, Env>(fa: HKT8<F, SI, SO, X, In, S, Env, E, A>) => B
}
...
export interface ReduceRightKE<F extends URIS, E> extends HasConstrainedE<F, E> {
  ...
  readonly reduce: <A, B>(
    b: B,
    f: (a: A, b: B) => B
  ) => <SI, SO, X, In, S, Env>(fa: Kind<F, SI, SO, X, In, S, Env, E, A>) => B
}
