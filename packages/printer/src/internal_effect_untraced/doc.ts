...
import * as _flatten from "@effect/printer/internal_effect_untraced/flatten"
...
const protoHash = {
  ...
  FlatAlt: (self: Doc.FlatAlt<any>) =>
    Hash.combine(Hash.hash(DocSymbolKey))(Hash.combine(Hash.hash(self.left))(Hash.hash(self.right))),
  Cat: (self: Doc.Cat<any>) =>
    Hash.combine(Hash.hash(DocSymbolKey))(Hash.combine(Hash.hash(self.left))(Hash.hash(self.right))),
  Nest: (self: Doc.Nest<any>) =>
    Hash.combine(Hash.hash(DocSymbolKey))(Hash.combine(Hash.hash(self.indent))(Hash.hash(self.doc))),
  ...
  Annotated: (self: Doc.Annotated<any>) =>
    Hash.combine(Hash.hash(DocSymbolKey))(Hash.combine(Hash.hash(self.annotation))(Hash.hash(self.doc)))
} as const
...
export const concatWith = dual<
  <A>(f: (left: Doc.Doc<A>, right: Doc.Doc<A>) => Doc.Doc<A>) => (docs: Iterable<Doc.Doc<A>>) => Doc.Doc<A>,
  <A>(docs: Iterable<Doc.Doc<A>>, f: (left: Doc.Doc<A>, right: Doc.Doc<A>) => Doc.Doc<A>) => Doc.Doc<A>
>(2, (docs, f) => {
...
const changesUponFlatteningSafe = <A>(self: Doc.Doc<A>): Effect.Effect<never, never, Flatten.Flatten<Doc.Doc<A>>> => {
  switch (self._tag) {
    case "Fail": {
    ...
    case "WithPageWidth": {
      return Effect.succeed(_flatten.flattened(pageWidth((pageWidth) => flatten(self.react(pageWidth)))))
    }
    case "Nesting": {
