...
export type Client<A extends HttpApi.HttpApi.Any> = [A] extends
  [HttpApi.HttpApi<infer _Groups, infer _ApiError, infer _ApiErrorR>] ? Simplify<
    & {
      readonly [Group in Extract<_Groups, { readonly topLevel: false }> as HttpApiGroup.Name<Group>]: [Group] extends
    ...
    & {
      readonly [Method in Client.TopLevelMethods<_Groups, _ApiError> as Method[0]]: Method[1]
    }
  > :
  never
