...
> =>
  ApiRouter.unwrap((router) =>
    router.pipe(
      // TODO: Build global error encoder
  ...
  return HttpRouter.makeRoute(
    ...
    Effect.withFiberRuntime((fiber) => {
      ...
      return (decodePath._tag === "Some"
        ? Effect.orDie(decodePath.value(routeContext.params))
          ...
          decodePayload._tag === "Some"
            ? Effect.bind("payload", (_) =>
              requestPayload(request, urlParams).pipe(
                Effect.flatMap(decodePayload.value),
                Effect.orDie
              ))
            : identity,
...
export const group = <
  ...
  RX = never
>(
  api: Api.Api<ApiName, Groups>,
  groupName: Name,
  build: (
    handlers: Handlers<never, never, ApiGroup.ApiGroup.EndpointsWithName<Groups, Name>>
  ) =>
    | Handlers<ApiGroup.ApiGroup.ErrorWithName<Groups, Name>, RH>
    | Effect.Effect<Handlers<ApiGroup.ApiGroup.ErrorWithName<Groups, Name>, RH>, EX, RX>
...
<EG, RG>(
  ...
  EG | Exclude<E, ApiEndpoint.ApiEndpoint.ErrorWithName<Endpoints, Name>>,
