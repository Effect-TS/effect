...
const makeSharedWorker = () => {
  ...
  return {
    worker: worker as unknown as MessagePort,
...
describe("DenoWorkerRunner", () => {
  it.effect("removes the registered messageerror listener", () =>
    Effect.gen(function*() {
      const fake = makePort()
      const runner = yield* DenoWorkerRunner.make(fake.port).start()
  ...
  it.effect("emits disconnects for closed SharedWorker ports", () =>
    Effect.gen(function*() {
      ...
      const runner = yield* DenoWorkerRunner.make(sharedWorker.worker).start()
      const fiber = yield* Effect.forkChild(runner.run<void, never, never>(() => {}))
      yield* Effect.yieldNow
