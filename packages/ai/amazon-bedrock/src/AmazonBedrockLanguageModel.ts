 ...
 * @category Provider Options
 */
export type AmazonBedrockReasoningOptions = {
...
export declare namespace ProviderMetadata {
   ...
   */
  export interface Service {
    "reasoning": AmazonBedrockReasoningOptions
...
> = Effect.fnUntraced(function*(request, response, options) {
  ...
  const usageMetadata: Mutable<ProviderMetadata.Service["finish"]["usage"]> = {}
  const metadata: Mutable<ProviderMetadata.Service["finish"]> = {
    usage: usageMetadata
  }
  if (Predicate.isNotUndefined(response.trace)) {
    metadata.trace = response.trace
  }
  if (Predicate.isNotUndefined(response.usage.cacheWriteInputTokens)) {
    usageMetadata.cacheWriteInputTokens = response.usage.cacheWriteInputTokens
  }
        ...
        switch (event.type) {
            ...
            switch (delta.type) {
              ...
              case "reasoningContent": {
                if ("text" in delta.reasoningContent) {
                  ...
                  const metadata: AmazonBedrockReasoningOptions = {
                    type: "thinking",
          ...
          case "metadata": {
            ...
            if (Predicate.isNotUndefined(event.metadata.usage.cacheWriteInputTokens)) {
              usageMetadata.cacheWriteInputTokens = event.metadata.usage.cacheWriteInputTokens
            }
            if (Predicate.isNotUndefined(event.metadata.trace)) {
              metadata.trace = event.metadata.trace
            }
            break
          }
