...
export class AiResponse extends Schema.Class<AiResponse>(
  ...
  get finishReason(): FinishReason {
    const finishPart = this.parts.find((part) => part._tag === "Finish")
    return Predicate.isNotUndefined(finishPart)
      ? finishPart.reason
      : "unknown"
  }
...
export class ResponseMetadataPart extends Schema.TaggedClass<ResponseMetadataPart>(
   ...
   */
  timestamp: Schema.optional(Schema.ValidDateFromSelf)
}) {}
...
export class WithStructuredOutput<A> extends AiResponse {
  ...
  constructor(options: {
    /**
     * The original response produced by the model that should be augmented
     * with structured output results.
     */
    ...
    super({ role: "assistant", parts: options.response.parts })
 ...
 * @category Models
 */
export class WithToolCallResults<A> extends AiResponse {
  /**
   * @since 1.0.0
   */
  ...
  constructor(options: {
    ...
    this.encodedResults = options.encodedResults
  }
}
