...
) => (wa: Op.Option<A>) => Either<E, Separated<Op.Option<B>, Op.Option<C>>> = (f) => (
  wa
...
export const traverseTree: <A, E, B>(
  f: (a: A) => Either<E, B>
) => (ta: TR.Tree<A>) => Either<E, TR.Tree<B>> = (f) => (ta) =>
  TR.tree.traverse(either)(ta, f)
...
export const nonFailable = <S, R, E, A>(
  _: T.Effect<S, R, E, A>
): T.Effect<S, R, never, Either<E, A>> =>
  pipe(_, T.map(right), T.chainError(flow(left, T.pure)))
