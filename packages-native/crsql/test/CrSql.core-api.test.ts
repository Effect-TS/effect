...
layer(layers)((it) => {
  it.scoped("core: sha/site_id/db_version/next_db_version/rows_impacted", () =>
    Effect.gen(function*() {
      ...
      yield* TestHelpers.createTodosCrr
      const pk = "C0C1C2C3C4C5C6C7C8C9CACBCCCDCECF"
      yield* crsql.sql`INSERT INTO todos (id, content, completed) VALUES (unhex(${pk}), 'Hello', 0)`
  ...
  it.scoped("as_crr => tracked; as_table => tracking stops", () =>
    Effect.gen(function*() {
      ...
      const crsql1 = yield* CrSql.CrSql.fromSqliteClient({ sql })
  ...
  it.scoped("begin_alter/commit_alter: add column and capture thereafter", () =>
    Effect.gen(function*() {
      ...
      const crsql2 = yield* CrSql.CrSql.fromSqliteClient({ sql })
