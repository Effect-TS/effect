...
```ts
export declare const headOption: <A>(self: TArray<A>) => STM.STM<never, never, Option<A>>
```
...
```ts
export declare const lastOption: <A>(self: TArray<A>) => STM.STM<never, never, Option<A>>
```
