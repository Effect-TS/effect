...
describe("ReadonlyRecord", () => {
  ...
  it("traverseWithIndex", () => {
    ...
    assert.deepStrictEqual(
      getOrElse((): _.ReadonlyRecord<string, number> => _.empty)(t2),
      _.empty
    )
  })
  ...
  it("compact", () => {
    assert.deepStrictEqual(_.readonlyRecord.compact({ foo: none, bar: some(123) }), {
      bar: 123
    })
  })
  ...
  it("separate", () => {
    assert.deepStrictEqual(
      _.readonlyRecord.separate({ foo: left(123), bar: right(123) }),
      {
        left: { foo: 123 },
        right: { bar: 123 }
      }
