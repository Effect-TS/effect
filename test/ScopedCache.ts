  ...
  it.effect("get - concurrent use of the scoped effect returned by a single call to get should create only one resource", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        ...
        const acquire1 = Effect.provide(scoped, Context.make(Scope, scope1))
        const release1: Scope.Finalizer = (exit) => Scope.close(scope1, exit)
        const acquire2 = Effect.provide(scoped, Context.make(Scope, scope2))
    ...
    Effect.gen(function*($) {
      ...
      const [release1, release2] = yield* $(Effect.scoped(Effect.gen(function*($) {
        ...
        yield* $(Effect.provide(
          cache.get(void 0),
          Context.make(Scope, scope1)
        ))
        yield* $(Effect.provide(
          cache.get(void 0),
          Context.make(Scope, scope2)
        ))
        const release1: Scope.Finalizer = (exit) => Scope.close(scope1, exit)
        const release2: Scope.Finalizer = (exit) => Scope.close(scope2, exit)
  ...
  it.effect("get - when two scoped effects obtained by a single scoped effect returned by a single call to get live longer than the cache, the resource should be cleaned only when it is not in use anymore", () =>
    Effect.gen(function*($) {
      ...
      const [release1, release2] = yield* $(Effect.scoped(Effect.gen(function*($) {
        const cache = yield* $(scopedCache)
        const scoped = cache.get(void 0)
        yield* $(Effect.provide(scoped, Context.make(Scope, scope1)))
        yield* $(Effect.provide(scoped, Context.make(Scope, scope2)))
        const release1: Scope.Finalizer = (exit) => Scope.close(scope1, exit)
        const release2: Scope.Finalizer = (exit) => Scope.close(scope2, exit)
        return [release1, release2] as const
      })))
