...
Array query parameters that declare `style: "form"` (or omit `style`, which defaults to `"form"`) with `explode: false` are now serialized into a single comma-separated query value before URL encoding, so `["red", "blue"]` is sent as `?tags=red%2Cblue` instead of `?tags=red&tags=blue`. The generated method still accepts the same array, readonly array, element, and enum argument types.
