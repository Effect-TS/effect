...
- A `ScopedRef.set` in flight while the owning scope closed installed a value that was never released. Each generation's scope is now forked from the ref's own scope, which the owner closes. `ScopedRef.set` and `ScopedRef.fromAcquire` now interrupt if the owning scope is already closed.
