...
Remote volatile unary and stream requests retry routing when their handler returns an interrupt-only reply marked transient (shutdown, termination timeout or disconnect), unless any caller-interrupt marker is also present. The same request may execute its handler again, so side effects must tolerate re-execution. Caller-requested interrupts, including combined caller/transient interrupts, mixed failure/defect causes, persisted replies, graceful draining and local caller behavior are unchanged.
