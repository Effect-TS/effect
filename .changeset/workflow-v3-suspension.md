...
Avoid deadlocking memory workflows that complete their own awaited deferred. Self-completion schedules an interruptible wake in the engine scope so body cleanup can finish before replay and engine teardown can cancel the wake; external completion still waits for cleanup.
