...
- `maxAttempts` and a new `retrySchedule` option move from `take()` to `PersistedQueue.make()`. Attempts are now counted when an element is claimed, so handler metadata is 1-based and a crash on the final attempt still dead-letters instead of redelivering forever.
