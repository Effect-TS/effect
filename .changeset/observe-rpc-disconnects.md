...
export const make: <Rpcs extends Rpc.Any>(
  ...
  options?:
    | {
      ...
      readonly disableFatalDefects?: boolean | undefined
      /**
       * Called after the server handles a passive client disconnect. Callback
       * defects are logged and do not stop disconnect handling.
       */
      readonly onDisconnect?: ((clientId: number) => Effect.Effect<void>) | undefined
    }
    | undefined
) => Effect.Effect<
  never,
...
> = Effect.fnUntraced(function*<Rpcs extends Rpc.Any>(
  ...
  const handleDisconnect = onDisconnect === undefined
    ? server.disconnect
    : (clientId: number) =>
      Effect.andThen(
        server.disconnect(clientId),
        Effect.catchDefect(
          Effect.suspend(() => onDisconnect(clientId)),
          (defect) =>
            Effect.annotateLogs(Effect.logDebug(defect), {
              module: "RpcServer",
              method: "onDisconnect"
            })
        )
