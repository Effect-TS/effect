...
- `FindMyWay`: static routes match with a single map lookup, single trailing
  ...
  literals, already-canonical URLs skip the sanitization pipeline, and the
  per-request backtracking stack and empty query string results are no longer
  allocated.
- `HttpRouter`: route dispatch avoids the per-request context restore and
  interruptibility wrapper primitives, and `HttpRouter.params` resolves in a
  single effect operation.
- `HttpEffect.toHandled`: the request scope and uninterruptibility wrappers
  are fused into a single primitive, the server tracing middleware is skipped
  entirely when no tracing backend can observe the request, and without
  middleware or tracing the response handling runs as a single continuation
  frame instead of a `matchCauseEffect` + `onExit` pair.
- Fiber runtime: the run loop reads the fiber cache once per operation,
  continuation lookup avoids a duplicate ensure-continuation read per stack
  frame, and `Exit` detection uses a property read instead of the `in`
  operator.
